...
// Class that handles the InteractionTracker callbacks. Has a weak reference back to the ScrollingPresenter so it can be garbage-collected, 
...
{
public:
    InteractionTrackerOwner(const winrt::ScrollingPresenter& scrollingPresenter);
...
private:
    weak_ref<winrt::ScrollingPresenter> m_owner{ nullptr };
};
