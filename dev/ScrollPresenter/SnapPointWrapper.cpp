...
template <typename T>
SnapPointWrapper<T>::SnapPointWrapper(T const& snapPoint) : m_snapPoint(snapPoint)
{
}
    ...
    winrt::InteractionTracker const& interactionTracker, winrt::hstring const& target, winrt::hstring const& scale, bool isInertiaFromImpulse)
{
    m_restingValueExpressionAnimation = GetSnapPointFromWrapper(this)->CreateRestingPointExpression(
        m_ignoredValue, m_actualImpulseApplicableZone, interactionTracker, target, scale, isInertiaFromImpulse);
    ...
    winrt::InteractionTracker const& interactionTracker, winrt::hstring const& target, winrt::hstring const& scale, bool isInertiaFromImpulse)
{
    m_conditionExpressionAnimation = GetSnapPointFromWrapper(this)->CreateConditionalExpression(
        m_actualApplicableZone, m_actualImpulseApplicableZone, interactionTracker, target, scale, isInertiaFromImpulse);
...
std::tuple<winrt::ExpressionAnimation, winrt::ExpressionAnimation> SnapPointWrapper<T>::GetUpdatedExpressionAnimationsForImpulse()
{
    ...
    snapPoint->UpdateConditionalExpressionAnimationForImpulse(m_conditionExpressionAnimation, m_actualImpulseApplicableZone);
    snapPoint->UpdateRestingPointExpressionAnimationForImpulse(m_restingValueExpressionAnimation, m_ignoredValue, m_actualImpulseApplicableZone);
...
template <typename T>
std::tuple<winrt::ExpressionAnimation, winrt::ExpressionAnimation> SnapPointWrapper<T>::GetUpdatedExpressionAnimationsForImpulse(bool isInertiaFromImpulse)
{
    MUX_ASSERT(!SharedHelpers::IsRS5OrHigher());

    const SnapPointBase* snapPoint = GetSnapPointFromWrapper(this);

    snapPoint->UpdateExpressionAnimationForImpulse(m_conditionExpressionAnimation, isInertiaFromImpulse);
    snapPoint->UpdateExpressionAnimationForImpulse(m_restingValueExpressionAnimation, isInertiaFromImpulse);
...
void SnapPointWrapper<T>::DetermineActualApplicableZone(
    const SnapPointWrapper<T>* previousSnapPointWrapper, const SnapPointWrapper<T>* nextSnapPointWrapper, bool forImpulseOnly)
{
    ...
    if (!forImpulseOnly)
    {
        m_actualApplicableZone = snapPoint->DetermineActualApplicableZone(previousSnapPoint, nextSnapPoint);
    }

    m_actualImpulseApplicableZone = snapPoint->DetermineActualImpulseApplicableZone(
        previousSnapPoint, nextSnapPoint, m_ignoredValue, previousIgnoredValue, nextIgnoredValue);
}
...
template SnapPointBase* SnapPointWrapper<winrt::ScrollSnapPointBase>::GetSnapPointFromWrapper(
    std::shared_ptr<SnapPointWrapper<winrt::ScrollSnapPointBase>> snapPointWrapper);
template SnapPointBase* SnapPointWrapper<winrt::ZoomSnapPointBase>::GetSnapPointFromWrapper(
    std::shared_ptr<SnapPointWrapper<winrt::ZoomSnapPointBase>> snapPointWrapper);
