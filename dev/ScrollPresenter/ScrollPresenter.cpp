...
winrt::ScrollingEdgeScrollParameters ScrollPresenter::HorizontalEdgeScrollParameters()
{
...
winrt::ScrollingEdgeScrollParameters ScrollPresenter::VerticalEdgeScrollParameters()
{
...
int32_t ScrollPresenter::SetScrollVelocity(winrt::float2 offsetsVelocity)
{
    ...
    int32_t viewChangeCorrelationId = ChangeOffsetsWithVelocityPrivate(offsetsVelocity);
    ...
    bool justForPositionVelocityInPixelsPerSecond)
{
    ...
    SCROLLPRESENTER_TRACE_VERBOSE(*this, TRACE_MSG_METH_INT, METH_NAME, this, justForPositionVelocityInPixelsPerSecond);
    ...
    const winrt::PointerRoutedEventArgs& args)
{
    ...
    ProcessPointerEdgeScroll(pointerPoint, true /*raiseEdgeScrollQueued*/);
}
    ...
    const winrt::ScrollControllerScrollToRequestedEventArgs& args)
{
    ...
    {
        // Coalesce requests
        int32_t existingViewChangeCorrelationId = interactionTrackerAsyncOperation->GetViewChangeCorrelationId();
        std::shared_ptr<ViewChangeBase> viewChangeBase = interactionTrackerAsyncOperation->GetViewChangeBase();
    ...
    double viewportHeight)
{
    ...
    // TODO: potentially adjust constant velocity after viewportWidth/Height changed
    // TODO: decrease velocity when nearEdgeApplicableRange + farEdgeApplicableRange > viewportWidth/Height proratedly
...
int32_t ScrollPresenter::ChangeOffsetsWithVelocityPrivate(
    winrt::float2 offsetsVelocity)
{
    SCROLLPRESENTER_TRACE_INFO(*this, TRACE_MSG_METH_STR, METH_NAME, this,
        TypeLogging::Float2ToString(offsetsVelocity).c_str());
...
bool ScrollPresenter::ProcessPointerEdgeScroll(
...
std::shared_ptr<InteractionTrackerAsyncOperation> ScrollPresenter::GetLastInteractionTrackerOperation()
{
    if (!m_interactionTrackerAsyncOperations.empty())
    {
...
std::shared_ptr<InteractionTrackerAsyncOperation> ScrollPresenter::GetPendingInteractionTrackerOperationFromType(
...
std::shared_ptr<InteractionTrackerAsyncOperation> ScrollPresenter::GetInteractionTrackerOperationFromRequestId(
    int requestId) const 
{
    MUX_ASSERT(requestId >= 0);
    ...
    UINT pointerId)
{
    if (m_edgeScrollQueuedEventSource)
    {
        ...
        m_edgeScrollQueuedEventSource(*this, *edgeScrollQueuedEventArgs);
    }
}
