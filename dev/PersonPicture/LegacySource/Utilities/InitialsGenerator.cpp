...
// Implementation of the InitialsGenerator class.
...
/// </remarks>
String^ InitialsGenerator::InitialsFromContactObject(Contact^ contact)
{
    if (contact == nullptr)
    {
        return nullptr;
    ...
    if (!contact->FirstName->IsEmpty() && !contact->LastName->IsEmpty())
    {
        ...
        {
            // Return null. In our code-behind we will produce a generic glyph as a result.
            return nullptr;
        }
    }
...
String^ InitialsGenerator::InitialsFromDisplayName(String^ contactDisplayName)
{
    ...
    if (type == CharacterType::Standard)
    {
        ...
        std::vector<std::wstring> words = Split(displayName, wchar_t(L' '));
...
{
    // Index should begin at the first desierable character.
    ...
    // If no desierable characters exist, we'll start at index 1, as combining
...
CharacterType InitialsGenerator::GetCharacterType(Platform::String ^ str)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        case CharacterType::Glyph:
            // Glyph beats rock, paper, and scissors.
...
{
    // To ensure predictable behavior, we're currently operating on a white list of character sets.
    //
