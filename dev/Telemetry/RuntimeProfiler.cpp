...
#define DEFINE_PROFILEGROUP(name, group, size) CMethodProfileGroup<size> name(group)
...
public:
    ...
    CMethodProfileGroup(ProfileGroup group) : m_cMethods(0), m_group(group)
    {
    ...
    {
        static_assert(
            sizeof(LONG) == sizeof(UINT32),
            "Since we're using InterlockedIncrement, make sure that this is the same size independent of build flavors.");
    ...
    void FireEvent(bool bSuspend) noexcept override
    {
        ...
        for (UINT32 ii = 0; ii < cMethods; ii++)
        {
            ...
            if (0 != cHits)
            {
                ...
                else if ((STRSAFE_E_INSUFFICIENT_BUFFER == hr) || (STRSAFE_E_INVALID_PARAMETER == hr))
                {
...
BOOL CALLBACK InitializeRuntimeProfiler(PINIT_ONCE /* InitOnce */, PVOID /* Parameter */, PVOID* /* context */)
{
    ...
    try
    {
        winrt::Application::Current().Suspending(([](auto&, auto&) { FireEvent(true); }));
    }
