...
void NavigationViewItemPresenter::OnApplyTemplate()
{
    ...
    if (auto navigationViewItem = GetNavigationViewItem())
    {
        if (auto const expandCollapseChevron = GetTemplateChildT<winrt::Grid>(c_expandCollapseChevron, *this))
        {
            ...
            m_expandCollapseChevronTappedToken =
                expandCollapseChevron.Tapped({navigationViewItem, &NavigationViewItem::OnExpandCollapseChevronTapped});
        }
...
bool NavigationViewItemPresenter::GoToElementStateCore(winrt::hstring const& state, bool useTransitions)
{
    ...
    // If NavigationViewItemPresenter is used, two sets of VisualStateGroups are supported. One set is help to switch the style
    // and it's NavigationViewItemPresenter itself and defined in NavigationViewItem Another set is defined in style for
    // NavigationViewItemPresenter. OnLeftNavigation, OnTopNavigationPrimary, OnTopNavigationOverflow only apply to itself.
...
{
    // We increased the ContentPresenter margin to align it visually with the expand/collapse chevron. This updated margin is even
    // applied when the NavigationView is in a visual state where no expand/collapse chevrons are shown, leading to more content
    // being cut off than necessary. This is the case for top-level items when the NavigationView is in a compact mode and the
    // NavigationView pane is closed. To keep the original cutoff visual experience intact, we restore  the original
    // ContentPresenter margin for such top-level items only (children shown in a flyout will use the updated margin).
    const auto stateName = isClosedCompact && isTopLevelItem ? L"ClosedCompactAndTopLevelItem" : L"NotClosedCompactAndTopLevelItem";
