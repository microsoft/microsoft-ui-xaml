...
void BreadcrumbBarItem::InstantiateFlyout()
{
    ...
    if (const auto& button = m_button.get())
    {
        if (const auto& ellipsisFlyout = m_ellipsisFlyout.get())
        {
            ...
            m_ellipsisRepeaterElementIndexChangedRevoker = ellipsisItemsRepeater.ElementIndexChanged(
                winrt::auto_revoke, { this, &BreadcrumbBarItem::OnFlyoutElementIndexChangedEvent });
