...
void RevealBorderLight::EnsureCompositionResources()
{
    ...
    if (!m_sharedLight && !m_compositionSpotLight)
    {
        ...
        if (m_materialProperties)
        {
            sharedLightInsp = m_materialProperties.TryGetLight(
                m_isWideLight ? winrt::Windows::UI::LightType::RevealBorderWide : winrt::Windows::UI::LightType::RevealBorder, compositor);
        }
...
void RevealBorderLight::EnsureLocalLight()
{
    if (!m_compositionSpotLight)
    {
        ...
        m_offsetProps.InsertScalar(
            L"SpotlightHeight",
            m_isWideLight ? s_revealBorderSpotlightStateWide.Height.Value : s_revealBorderSpotlightState.Height.Value);
        SetSpotLightStateImmediate(
            m_compositionSpotLight, m_colorsProxy, m_offsetProps, m_isWideLight ? s_revealBorderSpotlightStateWide : s_revealBorderSpotlightState);
...
{
    if ((m_shouldLightBeOn == turnOn) || m_isDisabledByMaterialPolicy || !m_colorsProxy)
    {
        return;
    }
    ...
    else
    {
        winrt::CompositionScopedBatch scopedBatch =
            winrt::Window::Current().Compositor().CreateScopedBatch(winrt::CompositionBatchTypes::Animation);
        animateSpotLight();
        scopedBatch.End();
        ...
        if (lightWithEnabledProperty)
        {
            ...
            scopedBatch.Completed([strongThis, lightWithEnabledProperty](auto sender, auto args) {
...
void RevealBorderLight::HookupWindowPointerHandlers()
{
    ...
    if (m_coreWindow)
    {
        ...
        // Otherwise, it's possible for the handler to get invoked after we've unsubscribed from the event and the object has been
        // destroyed. See comments in UnhookWindowPointerHandlers() for more details.
        m_PointerEnteredToken = m_coreWindow.PointerEntered([strongThis](const winrt::CoreWindow&, const winrt::PointerEventArgs&) {
        ...
        m_PointerExitedToken = m_coreWindow.PointerExited([strongThis](const winrt::CoreWindow&, const winrt::PointerEventArgs&) {
        ...
        m_PointerMovedToken = m_coreWindow.PointerMoved([strongThis](const winrt::CoreWindow&, const winrt::PointerEventArgs& args) {
        ...
        m_PointerPressedToken = m_coreWindow.PointerPressed([strongThis](const winrt::CoreWindow&, const winrt::PointerEventArgs&) {
        ...
        m_PointerReleasedToken = m_coreWindow.PointerReleased([strongThis](const winrt::CoreWindow&, const winrt::PointerEventArgs& args) {
        ...
        m_PointerCaptureLostToken =
            m_coreWindow.PointerCaptureLost([strongThis](const winrt::CoreWindow&, const winrt::PointerEventArgs& args) {
...
void RevealBorderLight::UnhookWindowPointerHandlers()
{
    if (m_coreWindow)
    {
        ...
        // In this case, CoreWindow's list of handlers is locked and the scheduled PointerExited will fire even after beig
        // unsubscribed. Clear the token after unregistering so any suuch callbacks can be ignored.
