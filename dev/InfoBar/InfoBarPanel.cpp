...
winrt::Size InfoBarPanel::MeasureOverride(winrt::Size const& availableSize)
{
    ...
    for (winrt::UIElement const& child : children)
    {
        ...
        if (childDesiredSize.Width != 0 && childDesiredSize.Height != 0)
        {
            ...
            totalWidth += childDesiredSize.Width + (nItems > 0 ? (float)horizontalMargin.Left : 0) +
            ...
            totalHeight += childDesiredSize.Height + (nItems > 0 ? (float)verticalMargin.Top : 0) +
        ...
        for (winrt::UIElement const& child : Children())
        {
            if (auto childAsFe = child.try_as<winrt::FrameworkElement>())
            {
                ...
                if (desiredSize.Width != 0 && desiredSize.Height != 0)
                {
                    ...
                    child.Arrange(winrt::Rect{ (float)verticalOrientationPadding.Left + (float)verticalMargin.Left,
                                               verticalOffset,
                                               desiredSize.Width,
                                               desiredSize.Height });
        ...
        for (unsigned int i = 0; i < childCount; i++)
        {
            ...
            if (auto childAsFe = child.try_as<winrt::FrameworkElement>())
            {
                ...
                if (desiredSize.Width != 0 && desiredSize.Height != 0)
                {
                    ...
                    if (i < childCount - 1)
                    {
                        child.Arrange(winrt::Rect{ horizontalOffset,
                                                   (float)horizontalOrientationPadding.Top + (float)horizontalMargin.Top,
                                                   desiredSize.Width,
                                                   desiredSize.Height });
                    }
                    else
                    {
                        ...
                        child.Arrange(winrt::Rect{ horizontalOffset,
                                                   (float)horizontalOrientationPadding.Top + (float)horizontalMargin.Top,
                                                   std::max(desiredSize.Width, finalSize.Width - horizontalOffset),
                                                   desiredSize.Height });
                    }
