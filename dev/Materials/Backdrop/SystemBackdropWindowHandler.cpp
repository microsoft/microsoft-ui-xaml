...
BaseWindowHandler::BaseWindowHandler(ISystemBackdropController* owningController) : m_controller{owningController}
{
    ...
    m_powerManagerEventRevoker = winrt::Windows::System::Power::PowerManager::EnergySaverStatusChanged(
        winrt::auto_revoke, [weakThis = get_weak(), dispatcherQueue = m_dispatcherQueue](auto&&, auto&&) {
    ...
    m_capabilitiesEventRevoker = {
        m_capabilities, m_capabilities.Changed([weakThis = get_weak(), dispatcherQueue = m_dispatcherQueue](auto&&, auto&&) {
            dispatcherQueue.TryEnqueue([weakThis]() {
                auto strongThis = weakThis.get();
                if (strongThis)
                {
    ...
    m_uiSettingsEventRevoker = m_uiSettings.AdvancedEffectsEnabledChanged(
        winrt::auto_revoke, [weakThis = get_weak(), dispatcherQueue = m_dispatcherQueue](auto&&, auto&&) {
            dispatcherQueue.TryEnqueue([weakThis]() {
                auto strongThis = weakThis.get();
                if (strongThis)
...
CoreWindowHandler::CoreWindowHandler(ISystemBackdropController* owningController, winrt::Windows::UI::Core::CoreWindow const& coreWindow) :
    BaseWindowHandler(owningController), m_coreWindow(coreWindow)
{
...
XamlWindowHandler::XamlWindowHandler(ISystemBackdropController* owningController, winrt::Windows::UI::Xaml::Window const& xamlWindow) :
    CoreWindowHandler(owningController, xamlWindow.CoreWindow()), m_xamlWindow(xamlWindow)
{
    InitializePolicy();
...
void XamlWindowHandler::AttachToThemeChanged(bool retryOnNextTick)
{
    ...
    {
        m_themeEventRevoker = root.ActualThemeChanged(winrt::auto_revoke, [this](auto&&, auto&&) { RefreshTheme(); });
