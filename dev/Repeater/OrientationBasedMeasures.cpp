...
{
    return m_orientation == ScrollOrientation::Vertical ? rect.Y + rect.Height : rect.X + rect.Width;
}
...
{
    return m_orientation == ScrollOrientation::Vertical ? rect.X + rect.Width : rect.Y + rect.Height;
}

winrt::Rect OrientationBasedMeasures::MinorMajorRect(float minor, float major, float minorSize, float majorSize)
{
    return m_orientation == ScrollOrientation::Vertical ? winrt::Rect{minor, major, minorSize, majorSize}
                                                        : winrt::Rect{major, minor, majorSize, minorSize};
}

winrt::Point OrientationBasedMeasures::MinorMajorPoint(float minor, float major)
{
    return m_orientation == ScrollOrientation::Vertical ? winrt::Point(minor, major) : winrt::Point(major, minor);
}

winrt::Size OrientationBasedMeasures::MinorMajorSize(float minor, float major)
{
    return m_orientation == ScrollOrientation::Vertical ? winrt::Size(minor, major) : winrt::Size(major, minor);
}
