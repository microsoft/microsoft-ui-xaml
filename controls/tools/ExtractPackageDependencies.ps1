...
$dependencies = select-string -path $sourceFile -Pattern '<AppxLocation>(.*?)([^\\]+?)</AppxLocation>' | % { $_.Matches.Groups[2].Value } | Where { !($_ -imatch "(ARM64|x86|x64)") -or ($_ -imatch $platform) } | Sort-Object | Get-Unique
