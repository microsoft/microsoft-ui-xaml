...
namespace Microsoft.UI.Xaml.Tests.MUXControls.InteractionTests.Common
{
    public class FindElement
    {
        ...
        public static UIObject GetDesktopTopLevelWindow(string title)
        {
            var nameCondition = UICondition.CreateFromName(title);
            var desktopWindowCondition = UICondition.CreateFromClassName("WinUIDesktopWin32WindowClass"); // Win32 class name
            var topLevelWindowCondition = desktopWindowCondition.AndWith(nameCondition);

            UIObject source;

            using (AppLaunchWaiter waiter = new AppLaunchWaiter(topLevelWindowCondition))
            {
                waiter.TryWait(TimeSpan.FromSeconds(1));
        ...
        public static UIObject GetDescendantByName(UIObject parent, string name)
        {
            if (parent == null)
            {
            ...
            {
                throw name == null ? new ArgumentNullException("name") : new ArgumentException("name");
            }

            foreach (UIObject obj in parent.Descendants)
            {
                if (obj.Name == name)
                {
                    return obj;
            ...
            Log.Comment("GetDescendantByName - No matching descendant found for '" + name + "'");
            return null;
        }
    }
