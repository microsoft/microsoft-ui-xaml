...
namespace MUXControlsTestApp
{
    public class TestPage : Page
    {
        public TestPage()
        {
            ...
            ((IDictionary<object, object>)Resources.ThemeDictionaries["Default"])["WhichTheme"] = ApplicationTheme.Dark;
            Resources.ThemeDictionaries["Light"] = new ResourceDictionary();
            ((IDictionary<object, object>)Resources.ThemeDictionaries["Light"])["WhichTheme"] = ApplicationTheme.Light;
            Resources.ThemeDictionaries["HighContrast"] = new ResourceDictionary();
            ((IDictionary<object, object>)Resources.ThemeDictionaries["HighContrast"])["WhichTheme"] = ApplicationTheme.Light;
        }
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (!DesignModeHelpers.IsInDesignMode)
            {
                ...
                UpdateTestFrameVisibility();
            }
        }

        private Visibility? desiredTestFrameVisibility = null;

        protected void UpdateTestFrameVisibility(Visibility? visibility = null)
        {
            if (visibility.HasValue)
            {
                desiredTestFrameVisibility = visibility.Value;
            }

            if (desiredTestFrameVisibility.HasValue && XamlRoot != null)
            {
                var testFrame = XamlRoot.Content as TestFrame;
                testFrame.ChangeBarVisibility(desiredTestFrameVisibility.Value);
            }
        }
    }
}
