...
namespace MUXControlsTestApp.Utilities
{
    ...
    /// The values are obtained via Comp's IExpCompositionPropertyChanged which provides notifications 
    ...
    ///     >> It does this by using COMP's internal IExpCompositionPropertyChanged interface, which provides notificatons 
    ///        for animated property changes from the DWM.
    ///     >> Color animations not supported due to current IExpCompositionPropertyChanged limitation.
    ///
    /// </summary>
    class CompositionPropertyLogger
    {
        ...
        public enum LoggableType
        {
            Float = 0,
            Vector3,
            // Color      <-- Color change notifications not supported by IExpCompositionPropertyChanged. 
            //            <-- Color swizzling (eg color.R) also not supported, so it's not possible to map a supported type.
        ...
        public CompositionPropertyLogger(XamlRoot root)
        {
        ...
        [ComImport]
        [System.Runtime.InteropServices.Guid("391578C6-A913-5F43-B7CA-3A89A4B32814")]
        [ComVisible(true)]
        [InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
        public interface IExpCompositionPropertyChangedListener
        {
            void NotifyColorPropertyChanged(
              [In] CompositionObject target,
              [In] uint propertyId,
        ...
        //---------------------------------------------------------------------------------------------------
        // interface IExpCompositionPropertyChanged - a partner interface used to request callbacks
        //                                                      when the specified property changes in DWM.
        //---------------------------------------------------------------------------------------------------

        [ComImport]
        [ComVisible(true)]
        [System.Runtime.InteropServices.Guid("12B579A9-6A27-5CDE-A2A1-C557BB7DFDB3")]
        [InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
        public interface IExpCompositionPropertyChanged
        {
            void SetPropertyChangedListener(
                NotificationProperties propertyId, 
                IExpCompositionPropertyChangedListener callback);
        }

        class PropertyChangedListenerHelper : IExpCompositionPropertyChangedListener
        {
            private readonly CompositionPropertyLogger _logger;
            ...
            {
                Compositor myCompositior = CompositionTarget.GetCompositorForCurrentThread();
                ...
                IExpCompositionPropertyChanged notifyPartner = (IExpCompositionPropertyChanged)(object)proxyVisual;
            ...
            public void RemovePropertyChangedListener(CompositionObject co, string propertyName, LoggableType type)
            {
                ...
                if (proxyVisual != null)
                {
                    ...
                    var notifyPropertyPartner = (IExpCompositionPropertyChanged)proxyVisual;
