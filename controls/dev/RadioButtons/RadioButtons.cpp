...
void RadioButtons::OnGettingFocus(const winrt::IInspectable&, const winrt::GettingFocusEventArgs& args)
{
    if (auto const repeater = m_repeater.get())
    {
        ...
        if (inputDevice == winrt::FocusInputDeviceKind::Keyboard)
        {
            ...
            else if ((winrt::InputKeyboardSource::GetKeyStateForCurrentThread(winrt::VirtualKey::Control) & winrt::CoreVirtualKeyStates::Down) != winrt::CoreVirtualKeyStates::Down)
            {
...
void RadioButtons::OnChildPreviewKeyDown(const winrt::IInspectable&, const winrt::KeyRoutedEventArgs& args)
{
    ...
    case winrt::VirtualKey::Down:
        ...
        {
            if (winrt::FocusManager::TryMoveFocus(winrt::FocusNavigationDirection::Next, GetFindNextElementOptions(false /*useThisAsSearchRoot*/)))
            {
                return args.Handled(true);
            }
        }
    ...
    case winrt::VirtualKey::Up:
        ...
        {
            if (winrt::FocusManager::TryMoveFocus(winrt::FocusNavigationDirection::Previous, GetFindNextElementOptions(false /*useThisAsSearchRoot*/)))
            {
                return args.Handled(true);
            }
        }
    ...
    case winrt::VirtualKey::Right:
        if (winrt::FocusManager::TryMoveFocus(winrt::FocusNavigationDirection::Right,
                GetFindNextElementOptions(args.OriginalKey() != winrt::VirtualKey::GamepadDPadRight /*useThisAsSearchRoot*/)))
        {
            return args.Handled(true);
        }
        args.Handled(HandleEdgeCaseFocus(false, args.OriginalSource()));
    ...
    case winrt::VirtualKey::Left:
        if (winrt::FocusManager::TryMoveFocus(winrt::FocusNavigationDirection::Left,
            GetFindNextElementOptions(args.OriginalKey() != winrt::VirtualKey::GamepadDPadLeft /*useThisAsSearchRoot*/)))
        {
            return args.Handled(true);
        }
        args.Handled(HandleEdgeCaseFocus(true, args.OriginalSource()));
...
winrt::FindNextElementOptions RadioButtons::GetFindNextElementOptions(bool useThisAsSearchRoot)
{
    auto const findNextElementOptions = winrt::FindNextElementOptions{};
    findNextElementOptions.SearchRoot(useThisAsSearchRoot ? *this : this->XamlRoot().Content());
...
bool RadioButtons::MoveFocus(int indexIncrement)
{
    if (auto const repeater = m_repeater.get())
    {
        if (auto const focusedElement = winrt::FocusManager::GetFocusedElement(XamlRoot()).try_as<winrt::UIElement>())
        {
            ...
            if (focusedIndex >= 0)
            {
                ...
                while (focusedIndex >= 0 && focusedIndex < itemCount)
                {
                    ...
                    {
                        if (item.Focus(winrt::FocusState::Programmatic))
                        {
                            return true;
                        }
                    }
