...
void AnimatedVisualPlayer::AnimationPlay::Start()
{
    ...
    else
    {
        ...
        if (m_batch)
        {
            ...
            m_batchCompletedToken = m_batch.Completed([me_weak](winrt::IInspectable const&, winrt::CompositionBatchCompletedEventArgs const&)
                {
                    auto me = me_weak.lock();
                    ...
                    if (me->m_owner)
                    {
                        ...
                        if (me->m_owner->AnimationOptimization() == winrt::PlayerAnimationOptimization::Resources)
                        {
