    ...
    bool onUIThread)
{
    ...
    else
    {
        ...
        {
            dispatcherQueue.TryEnqueue(winrt::DispatcherQueueHandler(callback));
        }
    }
}
...
/*static*/ int MaterialHelper::BrushTemplates<T>::GetEffectiveDpi(T* instance)
{
    ...
    // We need the CompositionIsland, which isn't directly exposed from the XamlIsland, so we're taking a detour
    // through XamlIsland.AppContent. Note that the AppContent might be null, which would make us fall back to
    // the default behavior, even though the XamlIsland has a CompositionIsland hidden inside it.
    if (instance->m_associatedIsland
        && instance->m_associatedIsland.AppContent())
    {
