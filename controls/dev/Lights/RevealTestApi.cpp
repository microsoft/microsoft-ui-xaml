...
{
    auto root = winrt::Window::Current() ? winrt::Window::Current().Content() : nullptr;
    if (root)
    {
...
winrt::SpotLight RevealTestApi::GetBackgroundSpotlightProxy(winrt::ApplicationTheme theme)
{
    ...
    if (!s_backgroundSpotlightProxy[index])
    {
        ...
        s_pointerMovedRevoker = winrt::CoreWindow::GetForCurrentThread().PointerMoved(winrt::auto_revoke,
            [=](const winrt::CoreWindow&, const ::winrt::Windows::UI::Core::PointerEventArgs&)
            {
