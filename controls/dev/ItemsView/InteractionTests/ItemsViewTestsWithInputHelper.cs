    ...
    public class ItemsViewTestsWithInputHelper
    {
        ...
        public void ClickedItemBroughtIntoView()
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "ItemsView Tests", "navigateToInteractiveTests" }))
            {
                ...
                Verify.AreEqual(-1, currentElementIndex, "Verifying CurrentElementIndex is initially -1");
                ...
                Verify.AreEqual(5, currentElementIndex, "Verifying CurrentElementIndex is 5");
                ...
                currentElementIndex = GetCurrentElementIndex();
                Verify.AreEqual(0, currentElementIndex, "Verifying CurrentElementIndex is 0");
        ...
        private void TabIntoItemsViewWithoutCurrentItem(bool useShiftTab)
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "ItemsView Tests", "navigateToInteractiveTests" }))
            {
                ...
                if (useShiftTab)
                {
                    ...
                    Verify.AreEqual(21, currentElementIndex, "Verifying CurrentElementIndex is 21");
                }
                else
                {
                    ...
                    Verify.AreEqual(15, currentElementIndex, "Verifying CurrentElementIndex is 15");
                }
        ...
        private void KeyboardNavigationWithLayout(string layout, bool useLateralMoves)
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "ItemsView Tests", "navigateToInteractiveTests" }))
            {
                ...
                LogScrollViewInfo(itemsViewUIObject);
                currentElementIndex = GetCurrentElementIndex();
                Verify.AreEqual(49, currentElementIndex, "Verifying CurrentElementIndex is 49");
                ...
                LogScrollViewInfo(itemsViewUIObject);
                currentElementIndex = GetCurrentElementIndex();
                Verify.AreEqual(49, currentElementIndex, "Verifying CurrentElementIndex is still 49");
                ...
                if (useLateralMoves)
                {
                    ...
                    LogScrollViewInfo(itemsViewUIObject);
                    currentElementIndex = GetCurrentElementIndex();
                    Verify.AreEqual(1, currentElementIndex, "Verifying CurrentElementIndex is 1");
                }
                ...
                LogScrollViewInfo(itemsViewUIObject);
                currentElementIndex = GetCurrentElementIndex();
                Verify.AreNotEqual(0, currentElementIndex, "Verifying CurrentElementIndex is not 0");
                ...
                if (useLateralMoves)
                {
                    ...
                    currentElementIndex = GetCurrentElementIndex();
                    Verify.AreNotEqual(0, currentElementIndex, "Verifying CurrentElementIndex is not 0");
                    Verify.AreNotEqual(1, currentElementIndex, "Verifying CurrentElementIndex is not 1");
                }
                ...
                outputDebugStringComponentTypes: outputDebugStringComponentTypes))
            {
                using (var loggingHelper = new LoggingHelper(this, outputDebugStringComponentTypes))
                {
                    ...
                    if (key == Key.Up || key == Key.PageUp)
                    {
                        ...
                        LogScrollViewInfo(itemsViewUIObject);
                        currentElementIndex = GetCurrentElementIndex();
                        Verify.AreEqual(999, currentElementIndex, "Verifying CurrentElementIndex is 999");
                    }
                    ...
                    int expectedCurrentElementIndex = 0;
                    ...
                    if (layout == "LinedFlowLayout")
                    {
                        ...
                        {
                            case Key.Down:
                                expectedCurrentElementIndex = 48;
                                break;
                            case Key.Up:
                                expectedCurrentElementIndex = 951;
                                break;
                            case Key.PageDown:
                                expectedCurrentElementIndex = 138;
                                break;
                            case Key.PageUp:
                                expectedCurrentElementIndex = 861;
                                break;
                        }
                    }
                    else
                    {
                        ...
                        {
                            case Key.Down:
                                expectedCurrentElementIndex = 8;
                                break;
                            case Key.Up:
                                expectedCurrentElementIndex = 991;
                                break;
                            case Key.PageDown:
                                expectedCurrentElementIndex = 19;
                                break;
                            case Key.PageUp:
                                expectedCurrentElementIndex = 980;
                                break;
                        }
                    }

                    if (currentElementIndex != expectedCurrentElementIndex)
                    {
                        Log.Warning($"Unexpected current element index {currentElementIndex} instead of {expectedCurrentElementIndex}");
                        LogAndClearTraces();
                        Wait.ForIdle();
                    }
                    ...
                    Verify.AreEqual(expectedCurrentElementIndex, currentElementIndex, "Verifying CurrentElementIndex.");
        ...
        private void TabNavigationThroughItemsView(bool useItemContainer, string selectionMode)
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "ItemsView Tests", "navigateToInteractiveTests" }))
            {
                ...
                currentElementIndex = GetCurrentElementIndex();
                Verify.AreEqual(1, currentElementIndex, "Verifying CurrentElementIndex is still 1");
        ...
        {
            Log.Comment("Retrieving btnGetCurrentElementIndex");
            UIObject getCurrentElementIndexUIObject = FindElement.ById("btnGetCurrentElementIndex");
            Button getCurrentElementIndexButton = new Button(getCurrentElementIndexUIObject);
            Verify.IsNotNull(getCurrentElementIndexButton, "Verifying that btnGetCurrentElementIndex Button was found");

            getCurrentElementIndexButton.Invoke();
            Wait.ForIdle();

            Log.Comment("Retrieving txtCurrentElementIndex");
            UIObject currentElementIndexUIObject = FindElement.ById("txtCurrentElementIndex");
            Edit currentElementIndexTextBox = new Edit(currentElementIndexUIObject);
            Verify.IsNotNull(currentElementIndexTextBox, "Verifying that txtCurrentElementIndex TextBox was found");

            Log.Comment($"CurrentElementIndex={currentElementIndexTextBox.Value}");

            return int.Parse(currentElementIndexTextBox.Value);
        }
        ...
        private void LogScrollViewInfo(UIObject itemsViewUIObject, double? expectedVerticalOffset = null)
        {
            Log.Comment("Retrieving PART_ScrollView");
            UIObject scrollViewUIObject = itemsViewUIObject.FirstChild;
            Verify.IsNotNull(scrollViewUIObject, "Verifying that PART_ScrollView was found");
