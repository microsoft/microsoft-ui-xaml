...
void RepeaterLayoutContext::RecycleElementCore(winrt::UIElement const& element)
{
    ...
    ITEMSREPEATER_TRACE_INFO_DBG(nullptr, TRACE_MSG_METH_STR_INT, METH_NAME, this, L"RecycleElement:", owner->GetElementIndex(element));
    owner->ClearElementImpl(element);
}

winrt::Rect RepeaterLayoutContext::VisibleRectCore()
{
    return winrt::get_self<ItemsRepeater>(GetOwner())->VisibleWindow();
}
