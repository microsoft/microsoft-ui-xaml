...
int ElementManager::GetFirstRealizedDataIndex() const
{
...
winrt::UIElement ElementManager::GetAt(int realizedIndex)
{
    ...
    if (IsVirtualizingContext())
    {
        if (!m_realizedElements[realizedIndex])
        {
            ...
            ITEMSREPEATER_TRACE_INFO_DBG(nullptr, TRACE_MSG_METH_STR_INT, METH_NAME, this, L"Creating element for sentinal with data index:", dataIndex);
...
void ElementManager::EnsureElementRealized(bool forward, int dataIndex, const wstring_view& layoutId)
{
    if (IsDataIndexRealized(dataIndex) == false)
    {
        ...
        MUX_ASSERT(IsDataIndexRealized(dataIndex));
        ITEMSREPEATER_TRACE_INFO_DBG(nullptr, TRACE_MSG_METH_STR_STR_INT, METH_NAME, this, layoutId.data(), L"Created element for index:", dataIndex);
    }
}
