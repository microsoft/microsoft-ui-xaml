...
struct ReferenceTracker : public ImplT<D, I ..., ::IReferenceTrackerExtension>, public ITrackerHandleManager
{
    ...
    {
        // Add an extra ref to prevent a crash due to double deletion.  There can be multiple reasons why it's good to
        // add an extra "safety ref" during destruction, but here's one such example:
        // C++/WinRT calls final_release when the ref-count goes to zero, and we call DeleteInstanceOnUIThread so that
        // if we're not on the UIThread, we dispatch a task to the UI thread to do the final delete there.  The problem
        // is that it's possible that while this final delete is happening on the UI thread, the same object is being
        // processed during the ReferenceTracker walk, which does an AddRef/Release to get the current ref count of the
        // object (see ComposingTrackerExtensionWrapper::GetActualRefCount).  If the refcount is zero, the AddRef/Release
        // will trigger another deletion (another call to this function), and we'll be in a bad state because the object
        // gets deleted twice.  The fix is to just add an extra ref, so it won't go to zero again.
        // C++/WinRT actually does this trick as well in root_implements<>::NonDelegatingRelease, which calls this function.
        // But the dtor for root_implements<> does one extra release, taking the refcount back down to zero again, so this
        // doesn't fix our problem.
        self->AddRef();
    ...
    static void DeleteInstanceOnUIThread(std::unique_ptr<D>&& self) noexcept
    {
        ...
        if(!self->IsOnThread())
        {
            ...
            if(auto dispatcherQueue = static_cast<ReferenceTracker<D, ImplT, I...>*>(self.get())->m_dispatcherQueue)
            {
            ...
            {
                // We expect to always have a dispatcherQueue.  If m_dispatcherQueue is null here,
                // it may be that "self" is in the middle of getting cleaned up, and its dtor has
                // already run.
                MUX_ASSERT(FALSE);
            }
        }
