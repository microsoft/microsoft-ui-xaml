...
namespace Microsoft.UI.Xaml.Tests.MUXControls.InteractionTests
{
    ...
    public class WebView2Tests
    {
        ...
        public static void ClassInitialize(TestContext testContext)
        {
            ...
            {
                Verify.Fail("WebView2 tests must be run elevated, as they required write access to the registry.");
            }
        ...
        private static int GetSdkBuildVersion()
        {
            ...
            string dllPath = Path.Combine(Environment.CurrentDirectory , "test", dllName);
        ...
        private static void SetWebViewElementFocus(string target, uint tabs)
        {
            ...
            // As of the change in PR 6834432, calling Wait.ForIdle() when a WebView2 has focus causes the WebView2
            // to lose focus. The regular version of KeyboardHelper.PressKey() calls Wait.ForIdle(), so in tests where
            // the WebView2 has focus, PressKey() should be called with the skipWait parameter set to true.
            KeyboardHelper.PressKey(Key.Tab, ModifierKey.None, tabs, true);
            WaitForFocus(target);
        }
        ...
        [TestMethod]
        [TestProperty("TestSuite", "A")]
        [TestProperty("Ignore", "True")] // Disabled due to instability, re-enable with http://task.ms/45673900
        ...
        public void Focus_BasicTabTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                KeyboardHelper.PressKey(Key.Tab, ModifierKey.None, 1, true);
                WaitForFocus("w1");
        ...
        public void Focus_ReverseTabTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                KeyboardHelper.PressKey(Key.Tab, ModifierKey.Shift, 1, true);
                WaitForFocus("w2");
        ...
        public void Focus_MouseActivateTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                // TODO_WebView2Input - Task 30555367 - Stack Overflow in WebView2 pointer tests when (MUXC) test infra calls WV2.GetClickablePoint()
                //                      Workaround to avoid any calls to WebView2.GetClickablePoint
                var clickPoint = new Point(bounds.X + w2_offsetX, bounds.Y + w2_offsetY);
                PointerInput.Move(clickPoint);
                PointerInput.Press(PointerButtons.Primary);
                PointerInput.Release(PointerButtons.Primary);
        ...
        [TestMethod]
        [TestProperty("TestSuite", "B")]
        [TestProperty("Ignore", "True")] // Task 34034748: WebView2.MultipleWebviews_LanguageTest is unstable
        ...
        [TestMethod]
        [TestProperty("TestSuite", "B")]
        public void DragDropIntoWebView2Test()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ChooseTest("DragDropIntoWebView2Test");

                var wv2 = FindElement.ById("MyWebView2");
                Rectangle wv2_bounds = wv2.BoundingRectangle;
                Log.Comment("[MyWebView2] Bounds = X:{0}, Y:{1}, Width:{2}, Height:{3}", wv2_bounds.X, wv2_bounds.Y, wv2_bounds.Width, wv2_bounds.Height);

                // Drag & Drop Source - TextBlock with AllowDrop set
                var draggableTextTextBlock = new Edit(FindElement.ById("DraggableTextTextBlock"));
                var allowDropCheckBox = new CheckBox(FindElement.ById("AllowDropCheckbox"));
                allowDropCheckBox.Check();

                // Drag & Drop Target - HTML Input/Text control 'w2'
                var miscTextBox = new Edit(FindElement.ById("MiscTextBox"));
                WaitForTextBoxValue(miscTextBox, string.Empty, false /* match = false for not match */);
                string wv2CenterFullString = miscTextBox.Value;
                Verify.IsTrue(wv2CenterFullString.StartsWith("w2 CenterPoint:"), "w2 Center information is available");

                // Read center point of Input control reported by webpage (expected format: "w2 CenterPoint: (CenterX,CenterY)")
                string[] w2_xy = ((wv2CenterFullString.Split('(')[1]).Split(')')[0]).Split(',');
                int w2_centerX = int.Parse(w2_xy[0]);
                int w2_centerY = int.Parse(w2_xy[1]);
                Log.Comment("w2: CenterX=" + w2_centerX + ", CenterY=" + w2_centerY);

                // Final target offset computed by adjusting w2 CenterPoint from WV2 center to it's top/left corner
                int w2_effectiveOffsetX = w2_centerX - wv2_bounds.Width / 2;
                int w2_effectiveOffsetY = w2_centerY - wv2_bounds.Height / 2;

                Log.Comment("Drag and drop text from DraggableTextTextBlock (XAML) to w2 Input/text (Web)");
                InputHelper.DragToTarget(draggableTextTextBlock, wv2, w2_effectiveOffsetX, w2_effectiveOffsetY);
        ...
        public void BasicKeyboardTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                KeyboardHelper.PressKey(Key.Left, ModifierKey.None, 1, true);
                KeyboardHelper.PressKey(Key.Left, ModifierKey.None, 1, true);
                KeyboardHelper.PressKey(Key.Left, ModifierKey.None, 1, true);
                ...
                KeyboardHelper.PressKey(Key.Right, ModifierKey.None, 1, true);
                KeyboardHelper.PressKey(Key.Right, ModifierKey.None, 1, true);
                KeyboardHelper.PressKey(Key.Right, ModifierKey.None, 1, true);
                ...
                KeyboardHelper.PressKey(Key.Left, ModifierKey.Shift, 1, true);
                KeyboardHelper.PressKey(Key.Left, ModifierKey.Shift, 1, true);
                ...
                // On app side, validate pasted text
        ...
        public void CoreWebView2Initialized_FailedTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                var browserExecutableKey = GetBrowserExecutableFolderKey();
                Log.Comment("Setting key for {0}", TestApplicationInfo.MUXControlsTestApp.ProcessName);
                browserExecutableKey.SetValue(TestApplicationInfo.MUXControlsTestApp.ProcessName, "c:\\badpath");
        ...
        private static Win32.RegistryKey GetBrowserExecutableFolderKey()
        {
        ...
        [TestMethod]
        [TestProperty("TestSuite", "C")]
        [TestProperty("Ignore", "True")] // Disabled due to 37434508
        ...
        public void WindowHiddenTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                Log.Comment("Invoke minimize button");
                minimizeButton.Invoke();
                Wait.ForMilliseconds(1000);

                Log.Comment("Invoke maximize button");
                maximizeButton.Invoke();
                Wait.ForMilliseconds(1000);
        ...
        [TestMethod]
        [TestProperty("TestSuite", "D")]
        public static void AccessKeyTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                using (var xamlFocusWaiter = new FocusAcquiredWaiter("MyWebView2"))
                {
                ...
                KeyboardHelper.PressKey(Key.Tab, ModifierKey.None, 1, true); // tab to text box
                WaitForFocus("w1");
                KeyboardHelper.PressKey(Key.Tab, ModifierKey.None, 1, true); // tab to input button
                WaitForFocus("b1");
                KeyboardHelper.PressKey(Key.Tab, ModifierKey.None, 1, true); // tab to dropdown
                WaitForFocus("s1");
                using (var xamlFocusWaiter = new FocusAcquiredWaiter("TabStopButton2"))
                {
                    KeyboardHelper.PressKey(Key.Tab, ModifierKey.None, 1, true); // tab to tabStopButton2
                    xamlFocusWaiter.Wait();
                    Verify.IsTrue(x2.HasKeyboardFocus, "TabStopButton2 has keyboard focus");
                }
        ...
        [TestMethod]
        [TestProperty("TestSuite", "D")]
        [TestProperty("Ignore", "True")] // Bug 39699234: WebView2.HtmlDropdownTest fails after WinUI 2.8 port
        ...
        public void LifetimeTabTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToBasicWebView2" }))
            {
                ...
                /*  TODO investigate why this needs three tabs sometimes
        ...
        public void EdgeProcessFailedTest()
        {
            using (var setup = new WebView2TestSetupHelper(new[] { "WebView2 Tests", "navigateToCoreObjectsWebView2" }))
            {
                ...
                BeginSubTest("CoreProcessFailed Event Test", "Set bad source #1 (browser crash) -> Set good source -> Set bad source #2 (tab crash) -> Reload");
...
namespace WebView2Temporary
{
    ...
    public class WebView2RenderingVerifier
    {
        ...
        static void CheckWebViewPixel(string webViewName, double scale, uint targetPixelColor, IntPtr memoryHdc)
        {
            ...
            int yPosition = Convert.ToInt32(50 + (bounds.Y * scale));
