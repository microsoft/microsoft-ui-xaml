...
{
    auto instance = LifetimeHandler::GetDisplayRegionHelperInstance();
    auto content = winrt::Window::Current() ? winrt::Window::Current().Content() : nullptr;
    ...
    if (instance->m_simulateDisplayRegions)
    {
        ...
        if (auto fe = content.as<winrt::FrameworkElement>())
        {
...
winrt::Rect DisplayRegionHelper::WindowRect()
{
    ...
    {
        return winrt::Window::Current() ? winrt::Window::Current().Bounds() : winrt::Rect();
    }
}
