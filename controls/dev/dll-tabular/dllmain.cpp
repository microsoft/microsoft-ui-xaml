...
//
// When this DLL (Tabular) activates a type from MUX (Microsoft.ui.xaml.dll),
...
DllGetActivationFactory_t GetMuxActivationFactoryFn()
{
    if (!s_muxFactoryResolved.load(std::memory_order_acquire))
    {
        ...
        // Under concurrent first activation multiple threads can run this block; emit the
        // trace only once (first winner) so the ETL isn't spammed with duplicate events.
        if (!s_bypassTraceEmitted.exchange(true, std::memory_order_relaxed))
        {
            TraceLoggingWrite(
                g_hLoggingProvider,
                "MuxActivationBypass",
    ...
    void** factory) noexcept
{
    *factory = nullptr;
    ...
    // Try this DLL before namespace-derived DLL probes for future cross-namespace Tabular types.
    {
        ComPtr<IActivationFactory> selfFactory;
        if (SUCCEEDED(WINRT_GetActivationFactory(hstr, reinterpret_cast<void**>(selfFactory.GetAddressOf()))) && selfFactory)
        {
            const HRESULT selfHr = selfFactory->QueryInterface(
                reinterpret_cast<const IID&>(iid), factory);
            if (SUCCEEDED(selfHr))
            {
                return selfHr;
...
HRESULT WINAPI DllGetActivationFactory(_In_ HSTRING activatableClassId, _Out_ ::IActivationFactory** factory)
{
    ...
    {
        winrt::hstring resources{L"Microsoft.UI.Xaml.Controls.Tabular.TabularControlsResources"sv};
...
STDAPI_(void) DeinitializeTabular()
{
    XamlMetadataProvider::ClearTypes();
    MUXControlsFactory::Deinitialize();
    RuntimeProfiler::UninitializeRuntimeProfiler();
    // MUXC-parity block (inert here, see include above): no material helper in the Tabular binary.
#ifndef MUXCONTROLS_TABULAR
