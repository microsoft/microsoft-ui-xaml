...
namespace AnimatedVisuals
{
    sealed class ProgressRingDeterminate : IAnimatedVisualSource
    {
        ...
        {
            // WinUI 3 specific change: .NET 6's IsTypePresent call returns false here, causing a failure in LottieCustomSourceTest since it won't create
            // the animated visual for the ProgressRing and ProgressRingCustomLottieSourcePage.xaml's IsPlayingText text will be False instead of True
            // since there's no animated visual to play.
            /*
            if (!Windows.Foundation.Metadata.ApiInformation.IsTypePresent("Microsoft.UI.Composition.CompositionGeometricClip"))
            {
                throw new PlatformNotSupportedException("Could not find CompositionGeometricClip for ProgressRingDeterminate");
            }
            */
            return true;
        }
