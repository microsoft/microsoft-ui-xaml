...
void TextCommandBarFlyout::UpdateButtons()
{
    ...
    if (auto textBoxTarget = Target().try_as<winrt::ITextBox>())
    {
        proofingFlyout = textBoxTarget.ProofingMenuFlyout();
    }
    else if (auto richEditBoxTarget = Target().try_as<winrt::IRichEditBox>())
    {
    ...
    auto commandListForCutCopyPaste =
        thisAsFlyoutBase && thisAsFlyoutBase.InputDevicePrefersPrimaryCommands() ?
        PrimaryCommands() :
        SecondaryCommands();
...
TextControlButtons TextCommandBarFlyout::GetRichEditBoxButtonsToAdd(winrt::RichEditBox const& richEditBox)
{
    ...
    {
        const auto disabledFormattingAccelerators = richEditBox.DisabledFormattingAccelerators();
