...
CommandBarFlyout::CommandBarFlyout()
{
    ...
    if (winrt::IFlyoutBase thisAsFlyoutBase = *this)
    {
        thisAsFlyoutBase.ShouldConstrainToRootBounds(false);
        thisAsFlyoutBase.AreOpenCloseAnimationsEnabled(false);
    }
    ...
    Opening({
        [this](auto const&, auto const&)
        {
            ...
            winrt::IFlyoutBase thisAsFlyoutBase = *this;
            ...
            if (auto commandBar = m_commandBar.get())
            {
                ...
                if (AlwaysExpanded())
                {
                    commandBar.OverflowButtonVisibility(winrt::Microsoft::UI::Xaml::Controls::CommandBarOverflowButtonVisibility::Collapsed);
                    ShowMode(winrt::FlyoutShowMode::Standard);
                }
                else
                {
                    commandBar.OverflowButtonVisibility(winrt::Microsoft::UI::Xaml::Controls::CommandBarOverflowButtonVisibility::Auto);
                }
                SharedHelpers::QueueCallbackForCompositionRendering(
                    [strongThis = get_strong(), commandBar]
                    {
                        if (auto const commandBarFlyoutCommandBar = winrt::get_self<CommandBarFlyoutCommandBar>(commandBar))
                        {
                            ...
                            if (strongThis->ShowMode() == winrt::FlyoutShowMode::Standard)
                            {
...
winrt::Control CommandBarFlyout::CreatePresenter()
{
    ...
    auto const scopeGuard = gsl::finally([commandBar]()
        {
...
void CommandBarFlyout::AddDropShadow()
{
    ...
    {
        winrt::Microsoft::UI::Xaml::Media::ThemeShadow shadow;
...
{
    const auto commandBarElementDependencyPropertiesCount = s_commandBarElementDependencyPropertiesCount;
