...
void AnnotatedScrollBar::OnLabelsGridSizeChanged(const winrt::IInspectable& sender, const winrt::SizeChangedEventArgs& args)
{
    auto labelsGrid = m_labelsGrid.get();
    MUX_ASSERT(labelsGrid);

    LabelContentAreaSize(labelsGrid.ActualHeight());
}
...
void AnnotatedScrollBar::OnApplyTemplate()
{
    ...
    if (auto labelsGrid = m_labelsGrid.get())
    {
        m_labelsGridSizeChangedRevoker = SizeChanged(winrt::auto_revoke, { this, &AnnotatedScrollBar::OnLabelsGridSizeChanged });
    }
...
void AnnotatedScrollBar::UnhookHandlers(bool isFromDestructor)
{
    ...
    m_labelsGridSizeChangedRevoker.revoke();
...
void AnnotatedScrollBar::UpdatePositionsOfAnchoredElements()
{
    ...
    if (IsValueScrollOffset())
    {
        EnsurePanningInfo();

        m_panningInfo.get()->PanningElementOffsetMultiplier(static_cast<float>(-1.0 / ScrollOffsetToLabelOffsetFactor()));
    }

    QueueLayoutLabels(500 /*millisecondWait*/);

    // When the window is resized this does not get updated via the OnLabelsGridSizeChanged callback.
    // Update it here.
    if (auto labelsGrid = m_labelsGrid.get())
    {
        LabelContentAreaSize(labelsGrid.ActualHeight());
...
{
    return label.ScrollOffset() * ScrollOffsetToLabelOffsetFactor();
}
...
void AnnotatedScrollBar::UpdateScrollOffsetToLabelOffsetFactor()
{
    ...
    ScrollOffsetToLabelOffsetFactor(scrollOffsetToLabelOffsetFactor);
}
...
void AnnotatedScrollBar::OnVerticalGridPointerMoved(const winrt::IInspectable& sender, const winrt::PointerRoutedEventArgs& args)
{
    ...
    {
        double const newScrollOffset = offsetFromTop / ScrollOffsetToLabelOffsetFactor();
...
void AnnotatedScrollBar::OnVerticalGridPointerPressed(const winrt::IInspectable& sender, const winrt::PointerRoutedEventArgs& args)
{
    ...
    const double newScrollOffset = args.GetCurrentPoint(m_verticalGrid.get()).Position().Y / ScrollOffsetToLabelOffsetFactor();
...
void AnnotatedScrollBar::ScrollTo(double offset, winrt::AnnotatedScrollBarScrollingEventKind scrollingEventKind)
{
    ...
    if (!IsValueScrollOffset())
    {
        scrollOffset = RaiseScrollOffsetFromValueRequested(offset);
    }
...
double AnnotatedScrollBar::RaiseScrollOffsetFromValueRequested(double value)
{
    ANNOTATEDSCROLLBAR_TRACE_INFO(*this, TRACE_MSG_METH_DBL, METH_NAME, this);

    if (!m_scrollOffsetFromValueRequestedEventSource)
    {
        return value;
    }

    auto scrollOffsetFromValueRequestedEventArgs = winrt::make_self<AnnotatedScrollBarScrollOffsetFromValueRequestedEventArgs>(value);

    m_scrollOffsetFromValueRequestedEventSource(*this, *scrollOffsetFromValueRequestedEventArgs);
    return scrollOffsetFromValueRequestedEventArgs.as<winrt::AnnotatedScrollBarScrollOffsetFromValueRequestedEventArgs>().ScrollOffset();
}

double AnnotatedScrollBar::RaiseValueFromScrollOffsetRequested(double offset)
{
    ANNOTATEDSCROLLBAR_TRACE_INFO(*this, TRACE_MSG_METH_DBL, METH_NAME, this, offset);

    if (!m_valueFromScrollOffsetRequestedEventSource)
    {
        return offset;
    }

    auto valueFromScrollOffsetRequestedEventArgs = winrt::make_self<AnnotatedScrollBarValueFromScrollOffsetRequestedEventArgs>(offset);

    m_valueFromScrollOffsetRequestedEventSource(*this, *valueFromScrollOffsetRequestedEventArgs);
    return valueFromScrollOffsetRequestedEventArgs.as<winrt::AnnotatedScrollBarValueFromScrollOffsetRequestedEventArgs>().Value();
}
...
winrt::IInspectable AnnotatedScrollBar::RaiseDetailLabelRequested(double offsetFromTop)
{
    ...
    const auto value = std::min(offsetFromTop / ScrollOffsetToLabelOffsetFactor(), Maximum() + ViewportSize());
    ...
    double viewportLength)
{
    ...
    if (m_operationsCount == 0)
    {
        ...
        {
            Value(RaiseValueFromScrollOffsetRequested(offset));
        }
    }
}
