...
ItemContainerAutomationPeer::ItemContainerAutomationPeer(winrt::FrameworkElement const& owner)
...
winrt::IRawElementProviderSimple ItemContainerAutomationPeer::SelectionContainer()
{
    if (auto itemContainer = GetImpl())
    {
        auto const selectionContainer = [itemContainer]()
        {
            ...
            while (parent)
            {
                if (auto const parentAsUIElement = parent.try_as<winrt::UIElement>())
                {
                    if (auto const peer = winrt::FrameworkElementAutomationPeer::CreatePeerForElement(parentAsUIElement))
                    {
                        if (auto const selectionProvider = peer.try_as<winrt::ISelectionProvider>())
                        {
            ...
            return parent.try_as<winrt::UIElement>();
        }();
