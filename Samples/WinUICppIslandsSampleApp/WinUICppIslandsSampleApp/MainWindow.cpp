...
namespace
{
    ...
    void LogCurrentWindowsXamlManager()
    {
...
bool MainWindow::OnCreate(HWND, LPCREATESTRUCT)
{
    ...
    CreateButton(L"Create DesktopWindowXamlSource", IDM_CREATE_DWXS, [this](HWND) {
        ...
        try
        {
            // Every XAML thread needs its own DispatcherQueueController, even when XAML is
            // already running elsewhere in the process.
...
void MainWindow::ShutdownXaml()
{
    ...
    {
        s_winUIState = s_dispatcherQueueController ? WinUIState::Running : WinUIState::ShutdownComplete;
