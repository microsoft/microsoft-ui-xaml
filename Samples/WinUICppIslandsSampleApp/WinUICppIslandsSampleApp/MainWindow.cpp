...
namespace
{
    ...
    std::atomic<WinUIState> s_winUIState{ WinUIState::ShutdownComplete };
    std::mutex s_windowsXamlManagersMutex;
    std::vector<winrt::weak_ref<winrt::Microsoft::UI::Xaml::Hosting::WindowsXamlManager>> s_windowsXamlManagers;

    void RemoveWindowsXamlManager(void* managerIdentity)
    {
        std::lock_guard<std::mutex> lock(s_windowsXamlManagersMutex);
        for (auto managerIterator = s_windowsXamlManagers.begin(); managerIterator != s_windowsXamlManagers.end();)
        {
            ...
            if (!manager || winrt::get_abi(manager.as<IInspectable>()) == managerIdentity)
            {
                managerIterator = s_windowsXamlManagers.erase(managerIterator);
            }
            else
            {
                ++managerIterator;
    ...
    void ObserveCurrentWindowsXamlManager()
    {
        const auto currentManager =
            winrt::Microsoft::UI::Xaml::Hosting::WindowsXamlManager::GetForCurrentThread();
        if (!currentManager)
        {
            return;
        }
        ...
        for (auto managerIterator = s_windowsXamlManagers.begin(); managerIterator != s_windowsXamlManagers.end();)
        {
            ...
            else if (manager.as<IInspectable>() == currentManager.as<IInspectable>())
            {
                return;
            }
            else
        ...
        s_windowsXamlManagers.emplace_back(currentManager);

        auto managerIdentity = winrt::get_abi(currentManager.as<IInspectable>());
        currentManager.XamlShutdownCompletedOnThread(
            [managerIdentity](const auto&, const auto&)
            {
                RemoveWindowsXamlManager(managerIdentity);
            });
    }

    uint32_t WindowsXamlManagerCount()
    {
        std::lock_guard<std::mutex> lock(s_windowsXamlManagersMutex);
        for (auto managerIterator = s_windowsXamlManagers.begin(); managerIterator != s_windowsXamlManagers.end();)
        {
            if (!managerIterator->get())
            {
                managerIterator = s_windowsXamlManagers.erase(managerIterator);
            }
            else
        ...
        return static_cast<uint32_t>(s_windowsXamlManagers.size());
    }

    void EnsureProcessShutdownEventHandlers()
    {
...
[[nodiscard]] int APIENTRY MainWindow::Run(_In_ HINSTANCE hInstance, _In_ int nCmdShow)
{
    ...
    EnsureProcessShutdownEventHandlers();
...
bool MainWindow::OnCreate(HWND, LPCREATESTRUCT)
{
    ...
    CreateButton(L"Create DesktopWindowXamlSource", IDM_CREATE_DWXS, [this](HWND) {
        ...
        try
        {
            ...
            ObserveCurrentWindowsXamlManager();
...
{
    if (s_winUIState == WinUIState::ShuttingDown)
    {
        ::MessageBox(
            GetHandle(),
...
void MainWindow::OnTimer(HWND, UINT)
{
    ...
    {
        const wchar_t* winUIState = L"Running";
        switch (s_winUIState.load())
        {
        case WinUIState::ShuttingDown:
            winUIState = L"Shutting down";
            break;
        case WinUIState::ShutdownComplete:
            winUIState = L"Shutdown complete";
            break;
        }
        ...
        ::StringCchPrintf(
            str,
            ARRAYSIZE(str),
            L"Object counts | Application: %u | DispatcherQueueController: %u | WindowsXamlManager: %u | DesktopWindowXamlSource: %u | WinUI: %s",
            winrt::WinUICppIslandsSampleApp::implementation::App::InstanceCount(),
            s_dispatcherQueueControllerCount.load(),
            WindowsXamlManagerCount(),
