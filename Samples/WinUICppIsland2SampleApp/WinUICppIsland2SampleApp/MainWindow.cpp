...
winrt::com_ptr< winrt::WinUICppIsland2SampleApp::implementation::App> g_app{ nullptr };
...
void MainWindow::CreateButton(const wchar_t* name, DWORD id, std::function<void(HWND)> func, int x, int y, int w, int h)
{
    ...
    AddFocusableHwnd(newButton.hwnd);
}
...
bool MainWindow::OnCreate(HWND, LPCREATESTRUCT)
{
    ...
    CreateButton(L"XamlIsland WebView2", IDM_CREATE_XI2B, [this](HWND) {
        try
        {
            if (!XamlUtil::IsXamlRunningInProcess() && g_app == nullptr)
            {
                winrt::make<winrt::WinUICppIsland2SampleApp::implementation::App>().as(g_app);
            }

            size_t position = m_xamlIslands.size();

            auto webView = WebView2();
            winrt::Windows::Foundation::Uri uri{ L"http://www.bing.com" };
            webView.Source(uri);

            // Make a XamlIsland
            auto xamlIsland = CreateXamlIsland();
            xamlIsland.Content(webView);

            // Parent the XamlIsland object to current window
            Microsoft::UI::WindowId mainWndId = Microsoft::UI::GetWindowIdFromWindow(GetHandle());

            auto island = xamlIsland.ContentIsland();
            auto bridge = Microsoft::UI::Content::DesktopChildSiteBridge::Create(island.Root().Compositor(), mainWndId);
            AddFocusableBridge(bridge);
    ...
    CreateButton(L"XamlIsland Control Set", IDM_CREATE_XI2TB, [this](HWND) {
        ...
        try
        {
            ...
            xamlIsland.Content(stackPanel);
            xamlIsland.SystemBackdrop(MicaBackdrop());

            auto height = 250;
            auto width = 600;            
            RectInt32 rect = { 10, 80 + static_cast<int>((height + 5) * position), width, height };

            // Create the elements in the Stack Panel
    ...
    CreateButton(L"More...", 208, [this](HWND hwnd) {
        ...
        ::AppendMenuW(contextMenu, MF_STRING, IDM_CLOSE_ISLAND_HWND, L"Close HWND of all Island2");
...
void MainWindow::OnCommand(HWND, int id, HWND hwndCtl, UINT )
{
    ...
    {
    case IDM_START_APP:
        if (winrt::WinUICppIsland2SampleApp::implementation::App::s_hasExistedInProcess)
        {
    ...
    case IDM_CREATE_MODAL_WINDOW:
        ...
        break;
    case IDM_CLEAR_DWXS:
        // TODO - port to XamlIsland
        //this->ClearXamlIsland();
        break;
