...
#define DECLARE_AUTOHANDLE_CLASS(className, handlePolicy) \
    typedef AppTestAutomationHelpers::AutoHandle<handlePolicy> className; \
...
namespace AppTestAutomationHelpers {
    ...
    template <typename THandlePolicy>
    AutoHandle<THandlePolicy>::AutoHandle(AutoHandle&& other) noexcept
    ...
    template <typename THandlePolicy>
    AutoHandle<THandlePolicy>& AutoHandle<THandlePolicy>::operator=(AutoHandle&& other) noexcept
    {
        Attach(other.Detach());
        return *this;
    }
