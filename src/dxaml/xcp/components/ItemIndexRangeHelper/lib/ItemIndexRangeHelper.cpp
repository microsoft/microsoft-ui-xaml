...
unsigned int ItemIndexRangeHelper::GetContinousIndicesLengthStartingAtIndex(
    const std::vector<unsigned int>& indices,
    int startIndex)
{
    unsigned int size = indices.size();
    unsigned int startValue = indices[startIndex];
    unsigned int length = 1;

    for (unsigned int i = startIndex + 1; i < size; ++i, ++length)
    {
