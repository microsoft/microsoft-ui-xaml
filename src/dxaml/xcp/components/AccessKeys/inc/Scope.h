...
namespace AccessKeys {
    ...
    public:
        ...
        {
            const bool inputAccumulatorSurrogateFlag = inputAccumulator.length() > 0 && IS_HIGH_SURROGATE(inputAccumulator.back());
            ...
            *invokeResult = TryInvokeDirectMatch(keyToMatch);
            ...
            {
                if (IS_HIGH_SURROGATE(inputCharacter))
                {
            ...
            if (allowPartialMatching && inputAccumulatorSurrogateFlag && IS_LOW_SURROGATE(inputCharacter))
            {
        ...
        _Check_return_ HRESULT ProcessEscapeKey()
        {
            ...
            {
                const auto poppedChar = inputAccumulator.back();
                inputAccumulator.pop_back();
                if (inputAccumulator.length() > 0 && IS_SURROGATE_PAIR(inputAccumulator.back(), poppedChar))
                {
