    ...
    bool useTargetInformation)
{
    ...
    for (CUIElement* element : ancestorChain.m_vector)
    {
        ...
        if (element->OfTypeByIndex<KnownTypeIndex::XamlIslandRoot>())
        {
            // A XamlIslandRoot knows its accumulated scale. Once we have this scale, we just use it. No need to keep walking up the parent chain.
