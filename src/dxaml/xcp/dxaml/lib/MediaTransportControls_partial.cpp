...
MediaTransportControls::HookupPartsAndHandlers()
{
    ...
    {
        m_tpMediaPositionSlider.Cast<Slider>()->SetIsSeekSlider(TRUE);
...
MediaTransportControls::UpdatePlayPauseUI()
{
    ...
    if (m_transportControlsEnabled)
    {
        IFC(UpdatePlayPauseButton(m_tpPlayPauseButton.Cast<ButtonBase>()));
        IFC(UpdatePlayPauseButton(m_tpTHLeftSidePlayPauseButton.Cast<ButtonBase>()));
...
// Updates the play/pause button's automation Name and Tooltip text according to m_isPlaying, and raises the automation property change
// notification if the name was changed.
_Check_return_ HRESULT
MediaTransportControls::UpdatePlayPauseButton(
    _In_opt_ ButtonBase* playPauseButton)
{
    if (playPauseButton)
    {
        wrl_wrappers::HString oldAutomationName;
        wrl_wrappers::HString newAutomationName;

        IFC_RETURN(DirectUI::AutomationProperties::GetNameStatic(playPauseButton, oldAutomationName.ReleaseAndGetAddressOf()));
        IFC_RETURN(DXamlCore::GetCurrentNoCreate()->GetLocalizedResourceString(m_isPlaying ? UIA_MEDIA_PAUSE : UIA_MEDIA_PLAY, newAutomationName.ReleaseAndGetAddressOf()));

        if (oldAutomationName != newAutomationName)
        {
            IFC_RETURN(DirectUI::AutomationProperties::SetNameStatic(playPauseButton, newAutomationName));
            IFC_RETURN(UpdateTooltipText(playPauseButton, newAutomationName));

            if (AutomationPeer::ArePropertyChangedListeners())
            {
                ctl::ComPtr<IAutomationPeer> automationPeer;

                IFC_RETURN(playPauseButton->GetOrCreateAutomationPeer(&automationPeer));

                if (automationPeer)
                {
                    IFC_RETURN(automationPeer.Cast<AutomationPeer>()->RaisePropertyChangedEventById(UIAXcp::APNameProperty, oldAutomationName, newAutomationName));
                }
            }
        }
    }
...
MediaTransportControls::UpdatePositionUI()
{
    ...
    if (m_transportControlsEnabled && m_wrOwnerParent.Get())
    {
        ...
        if (m_tpMediaPositionSlider)
        {
            ...
            m_tpMediaPositionSlider.Cast<Slider>()->SetCurrentPosition(currentMediaPosition, m_naturalDuration.TimeSpan);
...
MediaTransportControls::SetFocusAfterEnteringFullWindowMode()
{
    ...
    if (!compact)
    {
        if (m_tpPlayPauseButton)
        {
            IFC_RETURN(m_tpPlayPauseButton.Cast<ButtonBase>()->Focus(xaml::FocusState_Programmatic, &focused));
            ASSERT(focused);
    ...
    else
    {
        if (m_tpTHLeftSidePlayPauseButton)
        {
            IFC_RETURN(m_tpTHLeftSidePlayPauseButton.Cast<ButtonBase>()->Focus(xaml::FocusState_Programmatic, &focused));
            ASSERT(focused);
            IFC_RETURN(put_TabNavigation(xaml_input::KeyboardNavigationMode_Cycle));
        }
        else
...
MediaTransportControls::RemoveFromFullWindowMediaRoot()
{
    ...
    if (m_transportControlsEnabled && spOwnerParent.Get())
    {
        ...
        IFC(GetXamlDispatcherNoRef()->RunAsync(
            MakeCallback(
                ctl::ComPtr<MediaTransportControls>(this),
                &MediaTransportControls::SetFocusAfterExitingFullWindowMode)));
    }
...
MediaTransportControls::SetFocusAfterExitingFullWindowMode()
{
    ...
    {
        BOOLEAN focused = FALSE;
        IFC_RETURN(m_tpFullWindowButton.Cast<ButtonBase>()->Focus(xaml::FocusState_Programmatic, &focused));
        ASSERT(focused);
    }
    else
    {
...
MediaTransportControls::UpdateTimeAutomationProperties()
{
    ...
    if (m_tpTimeElapsedElement && !ctl::is<xaml_controls::IContentControl>(m_tpTimeElapsedElement.Get()))
    {
        ...
        if (m_sourceLoaded && !m_isPlaying)
        {
            ...
            IFC_RETURN(strAutomationName.Concat(wrl_wrappers::HStringReference(L" "), strAutomationName));
