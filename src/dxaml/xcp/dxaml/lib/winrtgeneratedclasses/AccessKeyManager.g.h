// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See LICENSE in the project root for license information.
//------------------------------------------------------------------------
//
//  Abstract:
//
//      XAML types.
//      NOTE: This file was generated by a tool.
//
//------------------------------------------------------------------------

#pragma once




namespace DirectUI
{
    // Note that the ordering of the base types here is important - the base factory comes first, followed by all the
    // interfaces specific to this type.  By doing this, we allow every Factory's CreateInstance method to be more
    // COMDAT-folding-friendly.  Because this ensures that the first vfptr contains GetTypeIndex, it means that all
    // CreateInstance functions with the same base factory generate the same assembly instructions and thus will
    // fold together.  This is significant for binary size in Microsoft.UI.Xaml.dll so change this only with great
    // care.
    class __declspec(novtable) AccessKeyManagerFactory:
       public ctl::AbstractActivationFactory
        , public ABI::Microsoft::UI::Xaml::Input::IAccessKeyManagerStatics
        , public ABI::Microsoft::UI::Xaml::Input::IAccessKeyManagerStatics2
    {
        BEGIN_INTERFACE_MAP(AccessKeyManagerFactory, ctl::AbstractActivationFactory)
            INTERFACE_ENTRY(AccessKeyManagerFactory, ABI::Microsoft::UI::Xaml::Input::IAccessKeyManagerStatics)
            INTERFACE_ENTRY(AccessKeyManagerFactory, ABI::Microsoft::UI::Xaml::Input::IAccessKeyManagerStatics2)
        END_INTERFACE_MAP(AccessKeyManagerFactory, ctl::AbstractActivationFactory)

    public:
        // Factory methods.

        // Static properties.
        IFACEMETHOD(get_IsDisplayModeEnabled)(_Out_ BOOLEAN* pValue) override;
        IFACEMETHOD(get_AreKeyTipsEnabled)(_Out_ BOOLEAN* pValue) override;
        IFACEMETHOD(put_AreKeyTipsEnabled)(_In_ BOOLEAN value) override;

        // Dependency properties.

        // Attached properties.

        // Static methods.
        IFACEMETHOD(ExitDisplayMode)() override;
        IFACEMETHOD(EnterDisplayModeForXamlRoot)(_In_ ABI::Microsoft::UI::Xaml::IXamlRoot* pXamlRoot) override;

        // Static events.
        IFACEMETHOD(add_IsDisplayModeEnabledChanged)(_In_ ABI::Windows::Foundation::ITypedEventHandler<IInspectable*, IInspectable*>* pValue, _Out_ EventRegistrationToken* pToken) override;
        IFACEMETHOD(remove_IsDisplayModeEnabledChanged)(_In_ EventRegistrationToken token) override;

    protected:
        HRESULT QueryInterfaceImpl(_In_ REFIID iid, _Outptr_ void** ppObject) override;



    private:

        // Customized static properties.
         _Check_return_ HRESULT get_IsDisplayModeEnabledImpl(_Out_ BOOLEAN* pValue); 
         _Check_return_ HRESULT get_AreKeyTipsEnabledImpl(_Out_ BOOLEAN* pValue); 
         _Check_return_ HRESULT put_AreKeyTipsEnabledImpl(_In_ BOOLEAN value); 

        // Customized static  methods.
         _Check_return_ HRESULT ExitDisplayModeImpl(); 
         _Check_return_ HRESULT EnterDisplayModeForXamlRootImpl(_In_ ABI::Microsoft::UI::Xaml::IXamlRoot* pXamlRoot); 
    };
}
