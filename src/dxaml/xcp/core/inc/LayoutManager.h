...
public:
    ...
    {
        return m_layoutCycleWarningContextsCountdown != -1 && !m_isLayoutCycleLoggingSuspended;
    }
    ...
    auto SuspendLayoutCycleLogging()
    {
        bool prevLayoutCycleLoggingSuspended = m_isLayoutCycleLoggingSuspended;
        m_isLayoutCycleLoggingSuspended = true;
        return LayoutCycleLoggingRestorer(prevLayoutCycleLoggingSuspended, this);
    }

    class LayoutCycleLoggingRestorer
    {
    public:
        LayoutCycleLoggingRestorer(bool prevLayoutCycleLoggingSuspended, CLayoutManager *layoutManager) :
            m_prevLayoutCycleLoggingSuspended(prevLayoutCycleLoggingSuspended),
            m_layoutManager(layoutManager)
        {}

        ~LayoutCycleLoggingRestorer()
        {
            m_layoutManager->m_isLayoutCycleLoggingSuspended = m_prevLayoutCycleLoggingSuspended;
        }

    private:
        bool m_prevLayoutCycleLoggingSuspended;
        CLayoutManager* m_layoutManager;
    };

    bool ShouldReportDesiredSizeChanged(_In_ CUIElement* element);
...
private:
    ...
    bool            m_isLayoutCycleLoggingSuspended{ false };
    ...
    std::vector<CUIElement*> m_elementsIgnoreDesiredSizeChanged; // used during LayoutCycle logging to skip reporting ancestors
};
