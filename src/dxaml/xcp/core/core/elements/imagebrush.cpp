    ...
    )
{
    *ppBrush = nullptr;
    ...
    wrl::ComPtr<ID2D1DeviceContext5> d2dDeviceContext;
    uint32_t width;
    uint32_t height;
    IFC_RETURN(printParams.GetD2DRenderTarget()->GetDeviceContext(d2dDeviceContext.ReleaseAndGetAddressOf()));
    IFC_RETURN(GetID2D1SvgDocument(d2dDeviceContext.Get(), d2dSvgDocument, &width, &height));
