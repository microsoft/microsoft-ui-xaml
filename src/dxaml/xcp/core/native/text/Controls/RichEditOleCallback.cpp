    ...
    )
{
    ...
    IFC_RETURN(pView->TransformToVisual(nullptr, &transform));
    IFC_RETURN(transform->TransformPoints(&pointerPosition, &pointerPosition, 1));
    const float zoomScale = RootScale::GetRasterizationScaleForElement(pView);
    pointerPosition *= zoomScale; //RaiseContextMenuOpeningEvent expects scale applied
