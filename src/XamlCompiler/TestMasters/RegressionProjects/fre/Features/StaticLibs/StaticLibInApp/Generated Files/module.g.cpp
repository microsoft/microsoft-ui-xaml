...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"StaticLibInApp.MainPage"))
    {
        return winrt_make_StaticLibInApp_MainPage();
    }

    if (requal(name, L"StaticLibInApp.XamlMetaDataProvider"))
    {
        return winrt_make_StaticLibInApp_XamlMetaDataProvider();
    }
