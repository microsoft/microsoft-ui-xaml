...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"MetadataTestbedCppWinRT.MainPage"))
    {
        return winrt_make_MetadataTestbedCppWinRT_MainPage();
    }

    if (requal(name, L"MetadataTestbedCppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_MetadataTestbedCppWinRT_XamlMetaDataProvider();
    }
