...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"CppWinRT.MainPage"))
    {
        return winrt_make_CppWinRT_MainPage();
    }

    if (requal(name, L"CppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_CppWinRT_XamlMetaDataProvider();
    }
