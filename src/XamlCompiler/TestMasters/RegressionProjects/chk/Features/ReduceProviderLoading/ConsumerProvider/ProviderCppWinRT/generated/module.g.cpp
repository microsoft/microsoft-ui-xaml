...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"ProviderCppWinRT.MainPage"))
    {
        return winrt_make_ProviderCppWinRT_MainPage();
    }

    if (requal(name, L"ProviderCppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_ProviderCppWinRT_XamlMetaDataProvider();
    }
