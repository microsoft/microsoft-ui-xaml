...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"ConsumerCppWinRT.MainPage"))
    {
        return winrt_make_ConsumerCppWinRT_MainPage();
    }

    if (requal(name, L"ConsumerCppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_ConsumerCppWinRT_XamlMetaDataProvider();
    }
