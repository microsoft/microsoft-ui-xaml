...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"StaticControlsLib.BlankUserControl"))
    {
        return winrt_make_StaticControlsLib_BlankUserControl();
    }

    if (requal(name, L"StaticControlsLib.XamlMetaDataProvider"))
    {
        return winrt_make_StaticControlsLib_XamlMetaDataProvider();
    }

    if (requal(name, L"StaticLibInRuntimeComponent.AnotherBlankUserControl"))
    {
        return winrt_make_StaticLibInRuntimeComponent_AnotherBlankUserControl();
    }

    if (requal(name, L"StaticLibInRuntimeComponent.XamlMetaDataProvider"))
    {
        return winrt_make_StaticLibInRuntimeComponent_XamlMetaDataProvider();
    }
