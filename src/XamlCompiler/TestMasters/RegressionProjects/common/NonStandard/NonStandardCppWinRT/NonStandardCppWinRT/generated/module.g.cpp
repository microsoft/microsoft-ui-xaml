...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"NonStandardCppWinRT.BlankPage"))
    {
        return winrt_make_NonStandardCppWinRT_BlankPage();
    }

    if (requal(name, L"NonStandardCppWinRT.MainPage"))
    {
        return winrt_make_NonStandardCppWinRT_MainPage();
    }

    if (requal(name, L"NonStandardCppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_NonStandardCppWinRT_XamlMetaDataProvider();
    }
