...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"BindPhasingTestBedCppWinRT.ExtraInfo"))
    {
        return winrt_make_BindPhasingTestBedCppWinRT_ExtraInfo();
    }

    if (requal(name, L"BindPhasingTestBedCppWinRT.MainPage"))
    {
        return winrt_make_BindPhasingTestBedCppWinRT_MainPage();
    }

    if (requal(name, L"BindPhasingTestBedCppWinRT.MyInfo"))
    {
        return winrt_make_BindPhasingTestBedCppWinRT_MyInfo();
    }

    if (requal(name, L"BindPhasingTestBedCppWinRT.MyItem"))
    {
        return winrt_make_BindPhasingTestBedCppWinRT_MyItem();
    }

    if (requal(name, L"BindPhasingTestBedCppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_BindPhasingTestBedCppWinRT_XamlMetaDataProvider();
    }
