...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"CppWinRTExe.MainPage"))
    {
        return winrt_make_CppWinRTExe_MainPage();
    }

    if (requal(name, L"CppWinRTExe.XamlMetaDataProvider"))
    {
        return winrt_make_CppWinRTExe_XamlMetaDataProvider();
    }
