...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"CppWinRTComponent.Class"))
    {
        return winrt_make_CppWinRTComponent_Class();
    }

    if (requal(name, L"CppWinRTComponent.CppWinRTUserControlFromWinRTComponent"))
    {
        return winrt_make_CppWinRTComponent_CppWinRTUserControlFromWinRTComponent();
    }

    if (requal(name, L"CppWinRTComponent.XamlMetaDataProvider"))
    {
        return winrt_make_CppWinRTComponent_XamlMetaDataProvider();
    }
