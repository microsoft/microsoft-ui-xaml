...
int32_t WINRT_CALL WINRT_GetActivationFactory(void* classId, void** factory) noexcept
{
    try
    {
        *factory = nullptr;
        ...
        {
            *factory = winrt::detach_abi(winrt::make<winrt::LinkedMDControlsCppWinRT::factory_implementation::A>());
            return 0;
        }

        if (requal(name, L"LinkedMDControlsCppWinRT.B"))
        {
            *factory = winrt::detach_abi(winrt::make<winrt::LinkedMDControlsCppWinRT::factory_implementation::B>());
            return 0;
        }

        if (requal(name, L"LinkedMDControlsCppWinRT.XamlMetaDataProvider"))
        {
            *factory = winrt::detach_abi(winrt::make<winrt::LinkedMDControlsCppWinRT::factory_implementation::XamlMetaDataProvider>());
            return 0;
        }
