...
int32_t WINRT_CALL WINRT_GetActivationFactory(void* classId, void** factory) noexcept
{
    try
    {
        *factory = nullptr;
        ...
        {
            *factory = winrt::detach_abi(winrt::make<winrt::MarkupExtensionsCppWinRT::factory_implementation::MainPage>());
            return 0;
        }

        if (requal(name, L"MarkupExtensionsCppWinRT.XamlMetaDataProvider"))
        {
            *factory = winrt::detach_abi(winrt::make<winrt::MarkupExtensionsCppWinRT::factory_implementation::XamlMetaDataProvider>());
            return 0;
        }
