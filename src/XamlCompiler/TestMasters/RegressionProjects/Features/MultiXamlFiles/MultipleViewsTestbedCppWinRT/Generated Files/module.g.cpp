...
void* __stdcall winrt_get_activation_factory([[maybe_unused]] std::wstring_view const& name)
{
    ...
    if (requal(name, L"MultipleViewsTestbedCppWinRT.MainPage"))
    {
        return winrt_make_MultipleViewsTestbedCppWinRT_MainPage();
    }

    if (requal(name, L"MultipleViewsTestbedCppWinRT.Namespace"))
    {
        return winrt_make_MultipleViewsTestbedCppWinRT_Namespace();
    }

    if (requal(name, L"MultipleViewsTestbedCppWinRT.XamlMetaDataProvider"))
    {
        return winrt_make_MultipleViewsTestbedCppWinRT_XamlMetaDataProvider();
    }
