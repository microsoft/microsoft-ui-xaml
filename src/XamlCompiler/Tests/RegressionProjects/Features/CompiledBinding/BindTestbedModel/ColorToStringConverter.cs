...
namespace BindTestbedModel
{
    public sealed class ColorToStringConverter : IValueConverter
    {
        ...
        public object ConvertBack(object valueObj, Type targetType, object parameter, string language)
        {
            string src = (string)valueObj;
            for (int i = 0; i < colorValues.Length; i++)
            {
                if (src == colorNames[i])
                {
            ...
            if (src.StartsWith("#"))
            {
                return Windows.UI.Color.FromArgb(Byte.Parse(src.Substring(1, 2), System.Globalization.NumberStyles.HexNumber),
            ...
            // return black if it can't parse the color
            return Colors.Black;
        }
    }
}
