...
namespace MS.Internal.Xaml
{
    internal abstract class XamlContext
    {
        ...
        {
            return namespaceName != null && namespaceName.Contains("?");
        }
                                                    ...
                                                    string tagNamespace, string propUsageNamespace, bool tagIsRoot)
        {
            ...
            if ((IgnoreAPIInformation(propUsageNamespace) == IgnoreAPIInformation(tagNamespace))
                || (tagNamespace == null && propUsageNamespace != null && tagType.GetXamlNamespaces().Contains(IgnoreAPIInformation(propUsageNamespace))))
            {
                XamlType rootTagType = tagIsRoot ? tagType : null;

                // If property namespace != the tag namespaces, it means the property is tagged with a conditional.
                // We need to retrieve the property from the right namespace then, to maintain the API Information
                // associated with the property.
                XamlType propType = tagNamespace != propUsageNamespace && HasAPIInformation(propUsageNamespace)
                    ? this.GetXamlType(new XamlTypeName(propUsageNamespace, tagType.Name), true)
                    : tagType;

                property = this.GetXamlProperty(propType, propName.Name, rootTagType);
                ...
                if (property == null)
                {
                    property = this.GetXamlAttachableProperty(propType, propName.Name);
                }
            }
