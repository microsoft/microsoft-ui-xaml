...
// Internal Type subclass used during signature decoding to carry the "pinned" flag for local variables.
// Peeled off by SignatureUnwrap.Unwrap() after DecodeLocalSignature returns.
...
{
    /// <summary>
    /// Wraps a Type to indicate it is pinned (local variable modifier).
    /// SRM's ISignatureTypeProvider.GetPinnedType returns TType, so we encode
    /// the pinned flag into the Type itself and strip it later via SignatureUnwrap.
    /// </summary>
    internal sealed class SignaturePinnedType : Type
    {
        internal Type Underlying { get; }

        internal SignaturePinnedType(Type underlying)
        {
            Underlying = underlying ?? throw new ArgumentNullException(nameof(underlying));
        }

        // Forward all Type virtuals to Underlying.
        ...
        {
            if (obj is SignaturePinnedType other)
