...
NumberBox::NumberBox()
{
    ...
    this->RegisterPropertyChangedCallback(winrt::AutomationProperties::LabeledByProperty(), { this, &NumberBox::OnAutomationPropertiesLabeledByPropertyChanged });
}

void NumberBox::UnhookEvents()
{
    m_upButtonClickRevoker.revoke();
    m_downButtonClickRevoker.revoke();
    m_textBoxPreviewKeyDownRevoker.revoke();
    m_textBoxKeyDownRevoker.revoke();
    m_textBoxKeyUpRevoker.revoke();
    m_popupUpButtonClickRevoker.revoke();
    m_popupDownButtonClickRevoker.revoke();
    m_isEnabledChangedRevoker.revoke();
    m_textBoxLoadedRevoker.revoke();
}
...
void NumberBox::OnApplyTemplate()
{
    ...
    {
        m_textBoxLoadedRevoker = textBox.Loaded(winrt::auto_revoke, { this, &NumberBox::OnTextBoxLoaded });        
    }
    ...
    if (const auto popupRoot = GetTemplateChildT<winrt::UIElement>(c_numberBoxPopupContentRootName, controlProtected))
    {
        if (!popupRoot.Shadow())
        {
            ...
            popupRoot.Translation({ translation.x, translation.y, static_cast<float>(shadowDepth) });
        }
    }
...
void NumberBox::OnAutomationPropertiesLabeledByPropertyChanged(const winrt::DependencyObject&, const winrt::DependencyProperty&)
{
    ReevaluateForwardedUIAProperties();
}

void NumberBox::ReevaluateForwardedUIAProperties()
{
    if (const auto textBox = m_textBox.get())
    {
        // UIA Name
        ...
        // UIA LabeledBy
        const auto labeledBy = winrt::AutomationProperties::GetLabeledBy(*this);
        if (labeledBy)
        {
            winrt::AutomationProperties::SetLabeledBy(textBox, labeledBy);
        }
    }
}
