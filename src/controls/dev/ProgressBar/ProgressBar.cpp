...
#define WINAPPSDK_CHANGEID_56785908 56785908 //[Watson Failure] caused by ACCESS_VIOLATION_c0000005_Microsoft.UI.Xaml.Controls.dll!ProgressBar::SetProgressBarIndicatorWidth
...
double LayoutRound(const double value, const double scaleFactor)
{
...
void ProgressBar::SetProgressBarIndicatorWidth()
{
    ...
    if (auto&& progressBar = m_layoutRoot.get())
    {
        if (auto&& determinateProgressBarIndicator = m_determinateProgressBarIndicator.get())
        {
            ...
            if (WinAppSdk::Containment::IsChangeEnabled<WINAPPSDK_CHANGEID_56785908>())
            {
            ...
            const double paddingAndBorderWidth = (padding.Left + padding.Right) + roundedBorderWidth;
            const double maxIndicatorWidth = std::max(progressBarWidth - paddingAndBorderWidth, 0.0);
