...
namespace Microsoft.UI.Xaml.Tests.MUXControls.InteractionTests
{
    ...
    public class AnnotatedScrollBarInteractionTests
    {
        ...
        public void DetailLabelRequestedOffsetDoesNotExceedMaxPossibleValue()
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "AnnotatedScrollBar Tests", "navigateToSummary" }))
            {
                Log.Comment("Retrieving isDetailLabelEnabled checkbox");
                CheckBox isDetailLabelEnabled = new CheckBox(FindElement.ById("isDetailLabelEnabled"));
                Verify.IsNotNull(isDetailLabelEnabled, "Verifying that isDetailLabelEnabled checkbox was found");
                ...
                Log.Comment("Check isDetailLabelEnabled");
                isDetailLabelEnabled.Toggle();
                Wait.ForIdle();
                ...
                InputHelper.MoveMouse(VerticalDecrementRepeatButton, 0, -1*VerticalDecrementRepeatButton.BoundingRectangle.Height / 2 - 1);
                Wait.ForIdle();
        ...
        public void LabelsGrowToAccomodateLongText()
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "AnnotatedScrollBar Tests", "navigateToSummary" }))
            {
                ...
                Log.Comment("Retrieving useLongLabelContent checkbox");
                CheckBox useLongLabelContent = new CheckBox(FindElement.ById("useLongLabelContent"));
                Verify.IsNotNull(useLongLabelContent, "Verifying that useLongLabelContent checkbox was found");

                Log.Comment("Verify scrollPresenter starting width.");
                Verify.AreEqual(376, scrollPresenter.BoundingRectangle.Width);

                Log.Comment("Check useLongLabelContent");
                useLongLabelContent.Toggle();
                Wait.ForMilliseconds(500);

                Log.Comment("Verify scrollPresenter ending width.");
                Verify.AreEqual(150, scrollPresenter.BoundingRectangle.Width);
            }
        ...
        public void VerifyCanCancelScrolling()
        {
            ...
            using (var setup = new TestSetupHelper(new[] { "AnnotatedScrollBar Tests", "navigateToSummary" }))
            {
                ...
                Log.Comment("Retrieving CancelScrollingEvent checkbox");
                CheckBox cancelScrollingEvent = new CheckBox(FindElement.ById("cancelScrollingEvent"));
                Verify.IsNotNull(cancelScrollingEvent, "Verifying that cancelScrollingEvent checkbox was found");

                Log.Comment("Verify Value is set to 0.");
                btnGetValue.Click();
                Verify.AreEqual(0.0, Convert.ToDouble(txtValue.DocumentText));

                Log.Comment("Check cancelScrollingEvent.");
                cancelScrollingEvent.Toggle();
                Wait.ForIdle();

                Log.Comment("Click VerticalDecrementRepeatButton.");
                VerticalDecrementRepeatButton.Click();

                Log.Comment("Uncheck cancelScrollingEvent.");
                cancelScrollingEvent.Toggle();
                Wait.ForIdle();
