    ...
    double maxBound)
{
    ...
    winrt::hstring originalColorName = winrt::ColorDisplayNameHelper::ToDisplayName(ColorFromRgba(HsvToRgb(originalHsv)));
        ...
        if (*newValue > maxBound)
        {
            ...
            else
            {
                *newValue = maxBound;
                ...
                newColorName = winrt::ColorDisplayNameHelper::ToDisplayName(ColorFromRgba(HsvToRgb(newHsv)));
                break;
            }
        }
    ...
    if (shouldFindMidPoint)
    {
        ...
        while (newColorName == currentColorName)
        {
            *currentValue += (direction == IncrementDirection::Lower ? -1 : 1) * incrementAmount;
            ...
            currentColorName = winrt::ColorDisplayNameHelper::ToDisplayName(ColorFromRgba(HsvToRgb(currentHsv)));
        }
...
void CreateCheckeredBackgroundAsync(
    ...
    winrt::DispatcherQueue &dispatcherQueue,
