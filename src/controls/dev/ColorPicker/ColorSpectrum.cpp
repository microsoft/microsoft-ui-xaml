...
void ColorSpectrum::OnApplyTemplate()
{
    ...
    {
        colorNameToolTip.Content(box_value(winrt::ColorDisplayNameHelper::ToDisplayName(Color())));
    }
...
void ColorSpectrum::OnKeyDown(winrt::KeyRoutedEventArgs const& args)
{
    ...
    const bool isControlDown = (winrt::InputKeyboardSource::GetKeyStateForCurrentThread(winrt::VirtualKey::Control) & winrt::CoreVirtualKeyStates::Down) == winrt::CoreVirtualKeyStates::Down;
...
void ColorSpectrum::RaiseColorChanged()
{
    ...
    if (colorChanged || areBothColorsBlack)
    {
        ...
        if (auto&& colorNameToolTip = m_colorNameToolTip.get())
        {
            colorNameToolTip.Content(box_value(winrt::ColorDisplayNameHelper::ToDisplayName(newColor)));
        }
...
void ColorSpectrum::CreateBitmapsAndColorMap()
{
    ...
    (winrt::IAsyncAction asyncInfo, winrt::AsyncStatus asyncStatus)
    {
        ...
        strongThis->DispatcherQueue().TryEnqueue(winrt::DispatcherQueueHandler(
