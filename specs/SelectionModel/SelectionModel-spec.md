...
The same four types - `SelectionModel`, `IndexPath`, `SelectionModelSelectionChangedEventArgs`
and `SelectionModelChildrenRequestedEventArgs` - exist in system XAML. The two surfaces were
compared member by member: they are identical, except for two members that WinUI exposes and
system XAML does not.

* `SelectionModelChildrenRequestedEventArgs.SourceIndex`
...
Every row below is a snippet of a small sample app, paired with the UI
