...
ListPickerFlyoutPresenterAutomationPeer::InitializeImpl(
    _In_ xaml_controls::IListPickerFlyoutPresenter* pOwner)
{
    HRESULT hr = S_OK;
    ...
    wrl::ComPtr<xaml::IFrameworkElement> spListPickerFlyoutPresenterAsFE;
    wrl::ComPtr<xaml_controls::IListPickerFlyoutPresenter> spOwner(pOwner);
    wrl::ComPtr<IInspectable> spInnerInspectable;

    ARG_NOTNULL(pOwner, "pOwner");

    IFC(ListPickerFlyoutPresenterAutomationPeerGenerated::InitializeImpl(pOwner));
    ...
    IFC((static_cast<IInspectable*>(pOwner))->QueryInterface<xaml::IFrameworkElement>(
        &spListPickerFlyoutPresenterAsFE));

    IFC(spInnerFactory->CreateInstanceWithOwner(
            spListPickerFlyoutPresenterAsFE.Get(),
            static_cast<xaml_automation_peers::IListPickerFlyoutPresenterAutomationPeer*>(this),
            &spInnerInspectable,
            &spInnerInstance));
...
_Check_return_ HRESULT
ListPickerFlyoutPresenterAutomationPeer::GetAutomationControlTypeCoreImpl(
...
_Check_return_ HRESULT
ListPickerFlyoutPresenterAutomationPeer::GetClassNameCoreImpl(
    _Out_ HSTRING *returnValue)
{
    HRESULT hr = S_OK;

    IFC(wrl_wrappers::HStringReference(L"ListPickerFlyoutPresenter").CopyTo(returnValue));
...
_Check_return_ HRESULT
ListPickerFlyoutPresenterAutomationPeer::GetNameCoreImpl(
    _Out_ HSTRING *returnValue)
{
    HRESULT hr = S_OK;
