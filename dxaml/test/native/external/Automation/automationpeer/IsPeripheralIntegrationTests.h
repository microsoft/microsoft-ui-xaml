...
namespace Microsoft { namespace UI { namespace Xaml { namespace Tests { namespace Automation { namespace AutomationPeer {
    ...
    class IsPeripheralIntegrationTests : public WEX::TestClass<IsPeripheralIntegrationTests>
    {
    public:
        BEGIN_TEST_CLASS(IsPeripheralIntegrationTests)
        ...
        BEGIN_TEST_METHOD(VerifyIsPeripheralSupplied_True)
            TEST_METHOD_PROPERTY(L"Description", L"Validate the case where UIA asks for IsPeripheral on an element with IsPeripheral=true")
        END_TEST_METHOD()

        BEGIN_TEST_METHOD(VerifyIsPeripheralSupplied_False)
            TEST_METHOD_PROPERTY(L"Description", L"Validate the case where UIA asks for IsPeripheral on an element with IsPeripheral=false")
        END_TEST_METHOD()

        BEGIN_TEST_METHOD(VerifyIsPeripheralNotSupplied)
            TEST_METHOD_PROPERTY(L"Description", L"Validate the case where UIA asks for IsPeripheral on an element and not providing a value for IsPeripheral")
    ...
    private:
        enum TestCase
        {
            IsPeripheralSupplied_True,
            IsPeripheralSupplied_False,
            IsPeripheralNotSupplied,
        };
        ...
        protected:
            bool IsPeripheralCore() override
            {
                return _IsPeripheralCore(m_tc);
            }
        ...
        static bool _IsPeripheralCore(TestCase test);
    };
