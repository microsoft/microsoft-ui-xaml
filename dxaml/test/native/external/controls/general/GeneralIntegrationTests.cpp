...
namespace Microsoft { namespace UI { namespace Xaml { namespace Tests { namespace Controls { namespace General {
    ...
    void GeneralIntegrationTests::VerifyCreationDoesNotQIOuter()
    {
        ...
        RunOnUIThread([&]()
        {
            ...
            LOG_OUTPUT(L"Validate Border");
            THROW_IF_FAILED(wf::GetActivationFactory(wrl::Wrappers::HStringReference(L"Microsoft.UI.Xaml.Controls.Border").Get(), reinterpret_cast<IInspectable**>(&factory)));
            safe_cast<xaml_controls::IBorderFactory^>(factory)->CreateInstance(reinterpret_cast<Platform::Object^>(static_cast<IInspectable*>(countingOuter.Get())), &inner);
            VERIFY_ARE_EQUAL(0, countingOuter.Get()->GetQueryInterfaceCallCount());
