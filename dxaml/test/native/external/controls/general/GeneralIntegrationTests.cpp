...
namespace Microsoft { namespace UI { namespace Xaml { namespace Tests { namespace Controls { namespace General {
    ...
    void GeneralIntegrationTests::VerifyCreationDoesNotQIOuter()
    {
        ...
        RunOnUIThread([&]()
        {
            ...
            LOG_OUTPUT(L"Validate VirtualizingStackPanel");
            THROW_IF_FAILED(wf::GetActivationFactory(wrl::Wrappers::HStringReference(L"Microsoft.UI.Xaml.Controls.VirtualizingStackPanel").Get(), reinterpret_cast<IInspectable**>(&factory)));
            safe_cast<xaml_controls::IVirtualizingStackPanelFactory^>(factory)->CreateInstance(reinterpret_cast<Platform::Object^>(static_cast<IInspectable*>(countingOuter.Get())), &inner);
            VERIFY_ARE_EQUAL(0, countingOuter.Get()->GetQueryInterfaceCallCount());
