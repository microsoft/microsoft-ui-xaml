...
namespace Microsoft { namespace UI { namespace Xaml { namespace Tests { namespace Framework { namespace Layering {
    ...
    class StrictModeTestsUap : public WEX::TestClass<StrictModeTestsUap>
    {
    public:
        BEGIN_TEST_CLASS(StrictModeTestsUap)
            TEST_CLASS_PROPERTY(L"BinaryUnderTest", L"Microsoft.UI.Xaml.dll")
            TEST_CLASS_PROPERTY(L"RunAs", L"UAP")
            TEST_CLASS_PROPERTY(L"Classification", L"Integration")
            TEST_CLASS_PROPERTY(L"MasterFile:ClassName", L"StrictModeTests")
        ...
        // WPF Dispatcher timing of the UI is vastly different than UWP
        END_TEST_METHOD()
    };
