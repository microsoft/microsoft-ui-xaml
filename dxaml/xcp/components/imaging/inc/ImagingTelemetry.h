...
public:
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM1(WaitForDownloadInProgress,
        uint64_t, Id,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM1(FoundCompletedDownload,
        uint64_t, Id,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // When we start parsing an encoded image's metadata
    DEFINE_TRACELOGGING_EVENT_PARAM1(ParseImageMetadataStart,
        uint64_t, Id,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // When we stop parsing an encoded image's metadata
    DEFINE_TRACELOGGING_EVENT_PARAM2(ParseImageMetadataStop,
        uint64_t, Id,
        HRESULT, parseHR,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // When an image download has completed, or we found previously cached bits so we don't need to download
    DEFINE_TRACELOGGING_EVENT_PARAM3(ImageDownloadCompleteNotification,
        uint64_t, Id,
        PCWSTR, Uri,
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(CreateImageCache,
        uint64_t, Id,
        PCWSTR, Uri,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // When we find an existing ImageCache with encoded bits
    DEFINE_TRACELOGGING_EVENT_PARAM2(FoundImageCache,
        uint64_t, Id,
        PCWSTR, Uri,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(CreateImageCacheFromExistingEncodedData,
        uint64_t, Id,
        PCWSTR, Uri,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // A request for a decode to render size has come in.
    DEFINE_TRACELOGGING_EVENT_PARAM5(RequestDecodeToRenderSize,
        uint64_t, Id,
        PCSTR, imageState,
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(DecodeToRenderSizeDisqualified,
        uint64_t, Id,
        PCSTR, reason,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // When a decode to render size starts.
    DEFINE_TRACELOGGING_EVENT_PARAM7(DecodeToRenderSizeStart,
        uint64_t, Id,
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM1(DecodeToRenderSizeStop,
        uint64_t, Id,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(QueueProcessDecodeRequests,
        uint64_t, Id,
        PCWSTR, Uri,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // The ImageCache running the ProcessDecodeRequests that was queued by QueueProcessDecodeRequests.
    DEFINE_TRACELOGGING_EVENT_PARAM3(ProcessDecodeRequests,
        uint64_t, Id,
        PCWSTR, Uri,
        uint32_t, State,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(QueueDecodeFromImageCache,
        uint64_t, Id,
        PCWSTR, Uri,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM1(QueueOffThreadDecode,
        uint64_t, Id,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));

    // The work running off-thread to decode the image. This work was queued by QueueOffThreadDecode.
    DEFINE_TRACELOGGING_EVENT_PARAM3(OffThreadDecodeStart,
        uint64_t, Id,
        PCWSTR, Uri,
    ...
    // The work running off-thread to decode the image. This work was queued by QueueOffThreadDecode.
    DEFINE_TRACELOGGING_EVENT_PARAM1(OffThreadDecodeStop,
        uint64_t, Id,
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE));
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(SoftwareBitmapFallbackAfterUploadError,
        uint64_t, Id,
    ...
    DEFINE_TRACELOGGING_EVENT_PARAM2(DecodeResultAvailable,
        uint64_t, Id,
