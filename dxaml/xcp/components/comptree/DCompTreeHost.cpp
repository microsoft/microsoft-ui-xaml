...
WUComp::ICompositionPathFactory* DCompTreeHost::GetPathFactory()
{
    if (m_pathFactory == nullptr)
    {
        wrl_wrappers::HStringReference strIWindowClassId(RuntimeClass_Microsoft_UI_Composition_CompositionPath);
        IFCFAILFAST(wf::GetActivationFactory(strIWindowClassId.Get(), &m_pathFactory));
    }

    return m_pathFactory.Get();
}
    ...
    if (m_easingFunctionStatics == nullptr)
    {
        ...
        else
        {
            IFCFAILFAST(wf::GetActivationFactory(wrl_wrappers::HStringReference(
                RuntimeClass_Microsoft_UI_Composition_CompositionEasingFunction).Get(), &m_easingFunctionStatics));
        }
    }
...
HRESULT DCompTreeHost::CreateWinRTInteropCompositionDevice(
    ...
    _Out_ void **ppDevice)
{
    wrl::ComPtr<WUComp::IInteropCompositorFactoryPartner> factoryPartner;
    if (WinAppSdk::Containment::IsChangeEnabled<WINAPPSDK_CHANGEID_46468883>())
    {
        factoryPartner = ActivationFactoryCache::GetActivationFactoryCache()->GetInteropCompositorFactoryPartner();
    }
    else
    {
        IFCFAILFAST(wf::GetActivationFactory(
            HStringReference(RuntimeClass_Microsoft_UI_Composition_Compositor).Get(),
            &factoryPartner));
    }
    ...
    IFCFAILFAST(factoryPartner->CreateInteropCompositor(nullptr /*pRenderingDevice*/, m_spInteropCompositorPartnerCallback, iid, ppDevice));
    ...
    if (targetHwnd != nullptr)
    {
        if (m_targetHwnd == nullptr)
        {
            ...
            if (!DesignerInterop::GetDesignerMode(DesignerMode::V2Only))
            {
                ...
                if (auto contentRoot =
                        DXamlServices::GetHandle()->GetContentRootCoordinator()->Unsafe_XamlIslandsIncompatible_CoreWindowContentRoot())
                {
...
_Check_return_ HRESULT DCompTreeHost::EnsureXamlIslandTargetRoots()
{
    for (auto& islandData : GetXamlIslandRenderData())
    {
        ...
        {
            // Create a WindowsPresentTarget for this XamlIsland
            ...
            IFC_RETURN(WindowsPresentTarget::CreateCompositedWindowlessPresentTarget(
                static_cast<CXcpDispatcher *>(xamlIslandRoot->GetContext()->GetHostSite()->GetXcpDispatcher())->GetHandle(),
...
_Check_return_ HRESULT DCompTreeHost::ConnectXamlIslandTargetRoots()
{
    ...
    for (auto& islandData : GetXamlIslandRenderData())
    {
        ...
        // Get the tree and visual for this XamlIsland
...
void DCompTreeHost::UpdateXamlIslandTargetSize(_In_ CXamlIslandRoot* xamlIslandRoot)
{
    if(xamlIslandRoot)
    {
        ...
        if (islandData != m_islandRenderData.end())
        {
            // If XamlIsland size is set before WindowsPresentTarget has been
...
{
    // Remove render data for XamlIsland, if it exists
    auto islandData = m_islandRenderData.find(xamlIslandRoot);
    if (islandData != m_islandRenderData.end())
    {
