...
bool CanHaveFocusableChildren<CDependencyObject>(_In_ CDependencyObject* const parent)
{
    ...
    FocusChildrenIteratorWrapper iterator = GetFocusChildrenInTabOrderIterator(parent);
    if (CDOCollection* collection = iterator.GetDOCollection())
    {
        if (collection->IsLeaving())
        {
            return false;
        }
    }
    ...
    {
        if (childNoRef && FocusProperties::IsVisible(childNoRef))
        {
            ...
            if (childNoRef->OfTypeByIndex<KnownTypeIndex::TextBlock>() ||
                childNoRef->OfTypeByIndex<KnownTypeIndex::RichTextBlock>() ||
                childNoRef->OfTypeByIndex<KnownTypeIndex::RichTextBlockOverflow>())
            {
                if (IsFocusable(childNoRef, false /*ignoreOffScreenPosition*/) && IsPotentialTabStop(childNoRef))
                {
                    return true;
                }
                else
                {
                    if (CanHaveFocusableChildren(childNoRef))
                    {
                        return true;
                    }
                }
            ...
            {
                if (IsFocusable(childNoRef, false /*ignoreOffScreenPosition*/))
                {
                    return true;
                }
                else
