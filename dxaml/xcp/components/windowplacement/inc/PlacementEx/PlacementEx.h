...
//
// See the windowplacement component README for source and usage information.
//

// The string-serialization and registry helpers (ToString, FromString,
// StoreInRegistry, FromRegistryKey, and friends) depend on C++ exceptions:
// wil::str_printf and wil::string_maker<std::wstring> are only declared when
// WIL_ENABLE_EXCEPTIONS is set, and FromString parses with a try/catch.
//
// WinUI compiles without exceptions and persists placement through the TLV blob
// in WindowPlacementBlob.h rather than a registry string, so those helpers are
// compiled out of the WinUI build. Other consumers that build with /EHsc get
// them unchanged. Define this to 0 to opt out explicitly.
#ifndef PLACEMENTEX_STRING_SERIALIZATION
#ifdef _CPPUNWIND
#define PLACEMENTEX_STRING_SERIALIZATION 1
#else
...
// Flags to adjust what PlacementEx::GetPlacement captures.
enum class CaptureFlags
{
    None                    = 0,

    SkipVirtualDesktopId    = 0x0001, // Don't query the window's virtual desktop id.
                                      // That query is a cross-apartment COM call. Made
                                      // from inside a window message handler it can
                                      // fail with RPC_E_CANTCALLOUT_ININPUTSYNCCALL, or
                                      // block the UI thread while the shell responds.
                                      // Callers capturing on the message path opt out
                                      // and lose only the virtual desktop, which is
                                      // best-effort state anyway.
};
DEFINE_ENUM_FLAG_OPERATORS(CaptureFlags);
...
public:
    ...
    static bool GetPlacement(
        ...
        CaptureFlags captureFlags = CaptureFlags::None);
    ...
    // Requires PLACEMENTEX_STRING_SERIALIZATION; see the note near ToString().
#if PLACEMENTEX_STRING_SERIALIZATION
    ...
    static void StorePlacementInRegistry(
        ...
        PCWSTR registryPath,
        PCWSTR registryKeyName);
#endif // PLACEMENTEX_STRING_SERIALIZATION
    ...
    //
    // These, and the registry helpers built on them, require C++ exceptions
    // (wil::str_printf and a try/catch parse loop). WinUI compiles without
    // exceptions and persists placement through the TLV blob in
    // WindowPlacementBlob.h, so they are compiled out there. See
    // PLACEMENTEX_STRING_SERIALIZATION at the top of this file.
#if PLACEMENTEX_STRING_SERIALIZATION
...
#ifdef USE_VIRTUAL_DESKTOP_APIS
    if (WI_IsFlagClear(captureFlags, CaptureFlags::SkipVirtualDesktopId) &&
        GetVirtualDesktopId(hwnd, &placement->virtualDesktopId))
    {
