...
// WINDOW_ACTION and friends are declared in the Windows SDK behind
// NTDDI_VERSION >= NTDDI_WIN11_GE. WinUI compiles at _WIN32_WINNT=0x0A00 so that it
// keeps running down-level, so the types come from the uplevel header instead. It is a
// no-op when the SDK already declares them.
#include <platformsdk-uplevel/winuser_windowaction.h>
