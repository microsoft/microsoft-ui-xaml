...
void ProjectedShadowManager::ReleaseResources()
{
    ...
    m_xamlIslandsGlobalScenes.clear();
...
bool ProjectedShadowManager::RemoveCaster(_In_ WUComp::IVisual* visual)
{
    ...
    for (auto& xamlIslandGlobalScenePair : m_xamlIslandsGlobalScenes)
    {
    ...
    _In_ CXamlIslandRoot* xamlIslandRoot)
{
    ...
    {
        //TO DO: what should we do for global scenes associated with xamlIslands ?
...
void ProjectedShadowManager::UpdateShadowScenes(_In_ CCoreServices* pCore)
{
    ...
    if (m_xamlIslandsGlobalScenes.size() > 0)
    {
        auto it = m_xamlIslandsGlobalScenes.begin();
        while (it != m_xamlIslandsGlobalScenes.end())
        {
            CXamlIslandRoot* xamlIslandRoot = it->first.lock_noref();
            xref_ptr<ThemeShadowGlobalScene> xamlIslandsGlobalScene = it->second;

            xamlIslandsGlobalScene->UpdateCasters();
            xamlIslandsGlobalScene->UpdateReceiversAndFilterCasters(pCore, xamlIslandRoot);

            if (AreCasterListsEmpty(xamlIslandsGlobalScene.get()))
            {
                it = m_xamlIslandsGlobalScenes.erase(it);
            }
            else
            {
                ++it;
    ...
    if (shadow != nullptr)
    {
        if (addToGlobalScene)
        {
            if (xamlIslandRoot != nullptr)
            {
                ...
                if (!m_xamlIslandsGlobalScenes[xamlIslandRootWeakRef])
                {
                    xref_ptr<ThemeShadowGlobalScene> xamlIslandsGlobalScene = nullptr;
                    xamlIslandsGlobalScene.attach(new ThemeShadowGlobalScene(m_dcompTreeHostNoRef, m_core));
                    xamlIslandsGlobalScene->EnsureInitialized(xamlIslandRoot);
                    m_xamlIslandsGlobalScenes[xamlIslandRootWeakRef] = xamlIslandsGlobalScene;
                }
                result = m_xamlIslandsGlobalScenes[xamlIslandRootWeakRef];
            }
            else
            {
                if (!m_globalScene)
...
bool ProjectedShadowManager::DoesPopupHaveGlobalCaster(_In_ CPopup* popup, _In_ CUIElement* uielement)
{
    ...
    if (m_xamlIslandsGlobalScenes.size() > 0)
    {
        for (const auto& xamlIslandGlobalScenePair : m_xamlIslandsGlobalScenes)
        {
