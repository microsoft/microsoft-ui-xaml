...
#include <platformsdk-uplevel\IUISettings6.h>
        ...
        if (auto source = ctl::query_interface_cast<wuv::IUISettings6>(uiSettings.Get()))
        {
            IFC_RETURN(source->add_AnimationsEnabledChanged(
                WRLHelper::MakeAgileCallback<wf::ITypedEventHandler<wuv::UISettings*, wuv::UISettingsAnimationsEnabledChangedEventArgs*>>(
                    ...
                    {
                        if (auto dispatcher = GetXamlDispatcherNoRef())
                        {
                            // This callback will be raised from a thread pool thread. Dispatch to UI Thread.
                            IFC_RETURN(dispatcher->RunAsync(MakeCallback(this, &DXamlCore::UpdateAnimationsEnabled)));
                        }
                        return S_OK;
                    }).Get(),
...
ixp::IContentIslandStatics*
DXamlCore::GetExpCompositionContentStatics()
{
    if (!m_spContentStatics)
    {
        IFCFAILFAST(ctl::GetActivationFactory(wrl_wrappers::HStringReference(
        RuntimeClass_Microsoft_UI_Content_ContentIsland).Get(),
        &m_spContentStatics));
    }

    return m_spContentStatics.Get();
}
...
_Check_return_ HRESULT DXamlCore::SetCoreWindow(_In_ wuc::ICoreWindow* pCoreWindow)
{
    ...
    m_hCore->GetInputServices()->SetApplicationHwnd(hwndCoreWindow);
...
HRESULT DXamlCore::InitializeInstanceFromIdle()
{
    ...
    if (coreServices != nullptr && !coreServices->IsCoreReady())
    {
        ...
        coreServices->GetInputServices()->SetApplicationHwnd(hwndCoreWindow);
    }
...
// ContentBounds are visible bounds or XamlIsland bounds if Xaml is hosted in island mode
...
// Returns the XamlIsland bounds if Xaml is in XamlIsland host mode. Some elements such as connected animations
// will always have a non-XamlIsland root and will never have XamlIsland bounds.
                ...
                VERIFYHR(source->add_AutoHideScrollBarsChanged(
                    WRLHelper::MakeAgileCallback<wf::ITypedEventHandler<wuv::UISettings*, wuv::UISettingsAutoHideScrollBarsChangedEventArgs*>>(
                        [this](wuv::IUISettings* sender, wuv::IUISettingsAutoHideScrollBarsChangedEventArgs* args) -> HRESULT
                        {
                            ...
                            {
                                // This callback will be raised from a thread pool thread. Dispatch to UI Thread.
                                IFC_RETURN(dispatcher->RunAsync(MakeCallback(this, &DXamlCore::OnAutoHideScrollbarsChanged)));
                            }
                            return S_OK;
