    ...
    _In_ wf::Size measureSize)
{
    ...
    {
        CalendarView::CalendarViewDayItemChangingEventSourceType* pEventSource = nullptr;
...
CalendarViewGeneratorMonthViewHost::RemoveToBeClearedContainer(_In_ CalendarViewDayItem* pContainer)
{
    HRESULT hr = S_OK;
    ...
    for (UINT current = toBeClearedContainerCount - 1; toBeClearedContainerCount > 0; --current)
    {
        ...
        if (spCurrentContainer.Get() == static_cast<ICalendarViewDayItem*>(pContainer))
        {
...
IFACEMETHODIMP CalendarViewGeneratorMonthViewHost::get_IsRegisteredForCallbacks(_Out_ BOOLEAN* pValue)
{
    *pValue = m_isRegisteredForCallbacks;
    return S_OK;
}

IFACEMETHODIMP CalendarViewGeneratorMonthViewHost::put_IsRegisteredForCallbacks(_In_ BOOLEAN value)
{
    m_isRegisteredForCallbacks = value;
    return S_OK;
}

_Check_return_ HRESULT CalendarViewGeneratorMonthViewHost::IsBuildTreeSuspended(_Out_ BOOLEAN* pReturnValue)
{
    auto pOwner = GetOwner()->GetHandle();
...
IFACEMETHODIMP CalendarViewGeneratorMonthViewHost::BuildTree(_Out_ BOOLEAN* pWorkLeft)
{
    HRESULT hr = S_OK;
    ...
    _In_ IInspectable* pItem)
{
    ...
    if (GetOwner()->ShouldRaiseEvent(KnownEventIndex::CalendarView_CalendarViewDayItemChanging))
    {
        ...
        IFC(spArgsConcrete->put_Callback(nullptr));
    ...
    _In_ const ctl::ComPtr<BudgetManager>& spBudget)
{
    ...
    for (UINT toClearIndex = containersToClearCount - 1; containersToClearCount > 0; --toClearIndex)
    {
        ...
        IFC(CalendarViewGeneratorHost::ClearContainerForItem(spContainer.Cast<CalendarViewDayItem>(), nullptr /*spItem*/));

        // potentially raise the event
        if (spContainer)
        {
            IFC(RaiseContainerContentChangingOnRecycle(spContainer.AsOrNull<IUIElement>().Get(), nullptr));
        }
    ...
    _In_ xaml_controls::ICalendarViewDayItemChangingEventArgs* pArgs)
{
    ...
    CalendarViewDayItemChangingEventArgs* pConcreteArgsNoRef = static_cast<CalendarViewDayItemChangingEventArgs*>(pArgs);
