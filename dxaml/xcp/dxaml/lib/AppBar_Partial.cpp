...
// Bug 55387860: [1.5 servicing] [File Explorer]:-[The option flyout in explorer pop up upwards and don't show all options]
#define WINAPPSDK_CHANGEID_55387860 55387860
...
AppBar::OnApplyTemplate()
{
    ...
    if (m_tpExpandButton)
    {
        ...
        ButtonBase* expandButtonBase = m_tpExpandButton.Cast<ButtonBase>();

        // Set a tooltip on the expand button.
        IFC_RETURN(SetExpandButtonToolTip(expandButtonBase, false /*isAppBarExpanded*/));
        ...
        IFC_RETURN(AutomationProperties::GetNameStatic(expandButtonBase, automationName.ReleaseAndGetAddressOf()));

        if (automationName.Get() == nullptr)
        {
            IFC_RETURN(SetExpandButtonAutomationName(expandButtonBase, false /*isAppBarExpanded*/));
        }
        else
        {
            // Since a custom Automation Name is set in Xaml markup, do not overwrite it here, or in AppBar::OnOpeningImpl / AppBar::OnClosingImpl.
            m_hasExpandButtonCustomAutomationName = true;
        }
    }
...
AppBar::OnOpeningImpl(_In_ IInspectable* pArgs)
{
    ...
    if (m_tpExpandButton)
    {
        ...
        IFC_RETURN(SetExpandButtonToolTip(expandButtonBase, true /*isAppBarExpanded*/));
        ...
        if (!m_hasExpandButtonCustomAutomationName)
        {
            ...
            IFC_RETURN(SetExpandButtonAutomationName(expandButtonBase, true /*isAppBarExpanded*/));
        }
    }
...
AppBar::GetShouldOpenUp(_Out_ bool* shouldOpenUp)
{
    ...
    *shouldOpenUp = (m_Mode == AppBarMode_Bottom);
    ...
    if (m_Mode == AppBarMode_Inline)
    {
        bool hasSpaceToOpenDown = true;
        IFC_RETURN(HasSpaceForAppBarToOpenDown(&hasSpaceToOpenDown));
...
_Check_return_ HRESULT AppBar::HasSpaceForAppBarToOpenDown(bool* hasSpace)
{
    ...
    wf::Point bottomOfExpandedAppBar = {};
    IFC_RETURN(transform->TransformPoint({ 0, static_cast<float>(contentHeight) }, &bottomOfExpandedAppBar));
    ...
    if (WinAppSdk::Containment::IsChangeEnabled<WINAPPSDK_CHANGEID_55387860>())
    {
...
_Check_return_ HRESULT
AppBar::SetExpandButtonAutomationName(_In_opt_ ButtonBase* expandButton, bool isAppBarExpanded)
{
    ASSERT(!m_hasExpandButtonCustomAutomationName);

    wrl_wrappers::HString appBarAutomationName;

    IFC_RETURN(AutomationProperties::GetNameStatic(this, appBarAutomationName.ReleaseAndGetAddressOf()));

    int expandButtonAutomationNameResourceId{};

    if (appBarAutomationName.Get() == nullptr)
    {
    ...
    wrl_wrappers::HString expandButtonAutomationName;

    IFC_RETURN(DXamlCore::GetCurrentNoCreate()->GetLocalizedResourceString(expandButtonAutomationNameResourceId, expandButtonAutomationName.ReleaseAndGetAddressOf()));
    ...
    if (appBarAutomationName.Get() != nullptr)
    {
        ...
        const int len = swprintf_s(
            ...
            expandButtonAutomationName.GetRawBuffer(nullptr),
            appBarAutomationName.GetRawBuffer(nullptr));

        IFCEXPECT_RETURN(len >= 0);

        IFC_RETURN(expandButtonAutomationName.Set(buffer));
    }

    IFC_RETURN(AutomationProperties::SetNameStatic(expandButton, expandButtonAutomationName.Get()));
...
_Check_return_ HRESULT
AppBar::SetExpandButtonToolTip(_In_opt_ ButtonBase* expandButton, bool isAppBarExpanded)
{
    wrl_wrappers::HString toolTipText;

    IFC_RETURN(DXamlCore::GetCurrent()->GetLocalizedResourceString(isAppBarExpanded ? TEXT_HUB_SEE_LESS : TEXT_HUB_SEE_MORE, toolTipText.ReleaseAndGetAddressOf()));

    ctl::ComPtr<IInspectable> boxedToolTipText;

    IFC_RETURN(PropertyValue::CreateFromString(toolTipText.Get(), &boxedToolTipText));

    IFC_RETURN(expandButton->SetValue(
