...
MediaTransportControls::SubscribeMediaPlayerEvents() noexcept
{
    if (m_spMediaPlayer.Get())
    {
        ...
        // sure this pointer still exists.  If weak reference is unreachable, we won't call into it
            ...
            [wrThis](wmp::IMediaPlayer*, IInspectable*) mutable -> HRESULT
        {
            ...
            {
                IFC_RETURN(spThis->GetXamlDispatcherNoRef()->RunAsync(MakeCallback(
                    spThis, &MediaTransportControls::HandleSeekCompleted)));
            }
            return S_OK;
        ...
        // Start assuming autoplay isn't enabled and that we need to rely on position changed events to udpate the UI.
        // If autoplay is enabled, we will unsubscribe with playback starts.
        IFC_RETURN(SubscribePositionChangedEvents());
...
MediaTransportControls::UnSubscribeMediaPlayerEvents() noexcept
{
    if (m_spMediaPlayer.Get())
    {
        ...
        IFC_RETURN(UnSubscribePositionChangedEvents());
...
_Check_return_ HRESULT
MediaTransportControls::SubscribePositionChangedEvents()
{
    IFC_RETURN(UnSubscribePositionChangedEvents());
    if (m_spMediaPlayer.Get())
    {
        ...
        IFC_RETURN(spPlaybackSession->add_PositionChanged(
            wrl::Callback< Microsoft::WRL::Implements<
            Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::ClassicCom>,
            wf::ITypedEventHandler<wmp::MediaPlaybackSession*, IInspectable*>,
            Microsoft::WRL::FtmBase >> (
                [wrThis](wmp::IMediaPlaybackSession*, IInspectable*) mutable -> HRESULT
        {
            ...
            {
                IFC_RETURN(spThis->GetXamlDispatcherNoRef()->RunAsync(MakeCallback(
                    spThis, &MediaTransportControls::OnMediaPropertyChanged, MediaPlayer_Position)));
            }
            return S_OK;
        }).Get(), &m_mediaPlayerPositionChangeToken));
    }
...
_Check_return_ HRESULT
MediaTransportControls::UnSubscribePositionChangedEvents()
{
    if (m_spMediaPlayer.Get() && m_mediaPlayerPositionChangeToken.value > 0)
    {
        ctl::ComPtr<wmp::IMediaPlayer3> spMediaPlayerExt;
        ctl::ComPtr<wmp::IMediaPlaybackSession> spPlaybackSession;
        ...
        IFC_RETURN(spPlaybackSession->remove_PositionChanged(m_mediaPlayerPositionChangeToken));
        ...
        switch (propertyIndex)
        {
            ...
            case MediaPlayer_Property::MediaPlayer_MediaBreak_CurrentState:
            {
                ...
                if (spPlaybackSession)
                {
                    ...
                    // If playing state changed, toggle position timer to avoid ticking if position is not constantly updating.
                    // Subscribe to position changed events while the position timer isn't active so we have some way
                    // of knowing when we seeked or skipped while paused and can update the UI accordingly.
                    if (previousIsPlaying != m_isPlaying)
                    {
                        if (m_isPlaying)
                        {
                            IFC(UnSubscribePositionChangedEvents());
                            IFC(StartPositionUpdateTimer());
                        }
                        else
                        {
                            IFC(SubscribePositionChangedEvents());
...
_Check_return_ HRESULT
MediaTransportControls::HandleSeekCompleted()
{
    if (m_controlPanelIsVisible)
    {
        // Update the UI position so the seek bar is fully updated for when we report the position using the automation peer.
    ...
    {
        // Report the slider position using the automation peer disregarding the state of event throttling.
        ...
        if (spAutomationPeer)
        {
            ...
            IFC_RETURN(spRangeBaseAutomationPeer.Cast<RangeBaseAutomationPeer>()->ForceRaiseValuePropertyChangedEvent());
        }
    }
