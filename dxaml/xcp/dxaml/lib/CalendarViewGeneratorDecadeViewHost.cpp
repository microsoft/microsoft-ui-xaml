...
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::GetContainer(
    ...
    _Outptr_ CalendarViewBaseItem** ppContainer)
{
    ...
    IFC(ctl::make<CalendarViewItem>(&spItem));
...
_Check_return_ IFACEMETHODIMP CalendarViewGeneratorDecadeViewHost::PrepareItemContainer(
...
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::GetIsFirstItemInScope(_In_ int index, _Out_ bool* pIsFirstItemInScope)
{
    HRESULT hr = S_OK;
...
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::GetUnit(_Out_ int* pValue)
{
    return GetCalendar()->get_Year(pValue);
}

_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::SetUnit(_In_ int value)
{
    return GetCalendar()->put_Year(value);
}

_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::AddUnits(_In_ int value)
{
    return GetCalendar()->AddYears(value);
}

_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::AddScopes(_In_ int value)
{
    if (value != 0)
    {
        auto pCalendar = GetCalendar();
...
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::GetFirstUnitInThisScope(_Out_ int* pValue)
{
    int year = 0;
    int firstYearInThisEra = 0;
    *pValue = 0;
...
// the virtual "Decade" scope doesn't exist in globalization calendar, when we adjust to the first/last
// unit in the decade scope, we need to make sure we don't cross the boundaries (Era).
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::GetLastUnitInThisScope(_Out_ int* pValue)
{
    int year = 0;
...
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::OnScopeChanged()
{
    wrl_wrappers::HString minYearString;
    wrl_wrappers::HString maxYearString;
    wrl_wrappers::HStringReference separator(L" - ");
    ...
    IFC_RETURN(minYearString.Concat(separator, tempResult));
    IFC_RETURN(tempResult.Concat(maxYearString, m_pHeaderText));  // "YYYY - YYYY"

    return S_OK;
}

_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::GetPossibleItemStrings(_Outptr_ const std::vector<wrl_wrappers::HString>** ppStrings)
{
...
_Check_return_ HRESULT CalendarViewGeneratorDecadeViewHost::CompareDate(_In_ wf::DateTime lhs, _In_ wf::DateTime rhs, _Out_ int* pResult)
{
    return GetOwner()->GetDateComparer()->CompareYear(lhs, rhs, pResult);
}

INT64 CalendarViewGeneratorDecadeViewHost::GetAverageTicksPerUnit()
{
