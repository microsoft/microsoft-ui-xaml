...
// Bug 46157957: CXamlIslandRoot calling InvalidateRect(NULL), which leads to unnecessary painting of desktop wallpaper on the creation of each explorer XAML island
#define WINAPPSDK_CHANGEID_46157957 46157957
...
void CJupiterControl::InvalidateView()
{
    if (!m_pWindow)
    {
        return;
    ...
    //
    if (IsInvalidateRectPresent())
    {
        InvalidateRect(m_pWindow->GetWindowHandle(), nullptr, FALSE);
    }
    else
    {
        ::RedrawWindow(m_pWindow->GetWindowHandle(), nullptr, nullptr, RDW_INTERNALPAINT);
    }
}
...
void CJupiterControl::OnDisplayChanged()
{
    ...
    if (WinAppSdk::Containment::IsChangeEnabled<WINAPPSDK_CHANGEID_46157957>())
    {
        if (pbh)
        {
            pbh->ForceRedraw();
    ...
    else
    {
        if (pbh)
        {
            // Propagate display change notification to browser host
            pbh->OnDisplayChange();
        }
        InvalidateView();
    }
...
HRESULT CJupiterControl::HandleSettingChangedMessage(_In_ LPARAM lParam)
{
    // TODO: Task 42409302: Update SettingsChanged event to include LPARAM info.
    // Once this is hooked up, this method can be called through the SettingsChanged event.

    if (lParam != NULL)
    {
        if (CompareStringOrdinal(reinterpret_cast<LPCWSTR>(lParam), -1, LOCALE_CHANGE_MSG, -1, FALSE) == CSTR_EQUAL)
        {
        ...
        else if (CompareStringOrdinal(reinterpret_cast<LPCWSTR>(lParam), -1, IMMERSIVE_COLORSET_CHANGE_MSG, -1, FALSE) == CSTR_EQUAL)
        {
