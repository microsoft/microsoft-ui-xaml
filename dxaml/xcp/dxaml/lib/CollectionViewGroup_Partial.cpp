...
_Check_return_
HRESULT
CollectionViewGroup::SetSource(_In_ IInspectable *pSource, _In_ IInspectable *pItems, _In_ GroupedDataCollectionView *pOwner)
{
    HRESULT hr = S_OK;
    ctl::ComPtr<wfc::IObservableVector<IInspectable *>> spItems;
    bool fSourceIsObservable = false;
...
void CollectionViewGroup::ClearOwner()
{
    // Forego the weak reference all together
    m_spOwnerRef = nullptr;
}

_Check_return_
HRESULT
CollectionViewGroup::GetObservableVector(
...
_Check_return_
HRESULT
CollectionViewGroup::OnItemsVectorChanged(_In_ wfc::IVectorChangedEventArgs *pArgs)
{
    HRESULT hr = S_OK;
