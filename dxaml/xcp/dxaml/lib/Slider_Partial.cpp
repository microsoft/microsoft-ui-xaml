...
{
    m_currentPosition.Duration = 0;
    m_currentDuration.Duration = 0;
}
...
IFACEMETHODIMP Slider::OnCreateAutomationPeer(_Outptr_ xaml_automation_peers::IAutomationPeer** ppAutomationPeer)
{
    ...
    if(m_isSeekSlider)
    {
        ctl::ComPtr<xaml_automation_peers::ISliderAutomationPeer> spSeekSliderAutomationPeer;
        ctl::ComPtr<IInspectable> spInner;

        IFC(DirectUI::SeekSliderAutomationPeer::CreateInstanceWithOwner(this,
            NULL,
            &spInner,
            &spSeekSliderAutomationPeer));
        IFC(spSeekSliderAutomationPeer.CopyTo(ppAutomationPeer));
    }
    else
    {
...
void Slider::SetIsSeekSlider(_In_ BOOLEAN isSeekSlider)
{
    m_isSeekSlider = isSeekSlider;
}

void Slider::SetCurrentPosition(_In_ const wf::TimeSpan& position, _In_ const wf::TimeSpan& duration)
{
    m_currentPosition = position;
    m_currentDuration = duration;
}

wf::TimeSpan Slider::GetCurrentPosition()
{
    return m_currentPosition;
}

wf::TimeSpan Slider::GetCurrentDuration()
{
