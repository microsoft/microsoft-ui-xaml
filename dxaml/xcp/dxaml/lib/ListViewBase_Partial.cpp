...
ListViewBase::ListViewBase()
    ...
    , m_wasItemOrHeaderFocussed(false)
    , m_isHolding(false)
    ...
    _Outptr_ DependencyObject** ppFocusable)
{
    ...
    *ppFocusable = nullptr;
    ...
    // IsTabNavigationWithVirtualizedItemsSupported()==True:
    ...
    if (!shouldCustomizeTabNavigation || (IsTabNavigationWithVirtualizedItemsSupported() && !isUsingTabNavigationOnce))
    {
        ...
        if (IsTabNavigationWithVirtualizedItemsSupported() && !isGrouping && shouldCustomizeTabNavigation && !isUsingTabNavigationOnce)
        {
        ...
        // If neither focused item nor focused group  exists, retrieve GroupIem for last focused item, if grouping.
        ...
        ASSERT(IsTabNavigationWithVirtualizedItemsSupported() || !scrollIntoViewAndPrepareContainer);
            ...
            !hasFocus)
        {
            ...
            if (!spGroupItem)
            {
                ...
                {
                    targetFocusedIndex = static_cast<INT>(m_lastFocusedIndex);
                }
            }
        }
    ...
    _Outptr_result_maybenull_ xaml::IDependencyObject** itemContainer)
{
    ...
    *itemContainer = itemContainerCandidate;
    ...
    if (!*itemContainer || isPositionedInGarbageSection)
    {
        ...
        if (IsTabNavigationWithVirtualizedItemsSupported() && *itemContainer)
        {
    ...
    _Out_ BOOLEAN* pIsCandidateTabStopOverridden)
{
    ...
    if (!isAncestor)
    {
        if (IsTabNavigationFullySupported())
        {
            xaml_input::KeyboardNavigationMode navigationMode;
...
// it's a Control which is visible, enabled and is either a tab stop or one of it's children is a tab stop.
_Check_return_ HRESULT ListViewBase::UpdateTabStopFlags()
{
    ...
    if (isGrouping)
    {
        ...
        if (groupCount > 0)
        {
            ...
            // loop across the cached header containers to find the first group header that can focused
...
{
    ASSERT(IsTabNavigationFullySupported());
    ...
    case Footer:
        {
            ...
            else
            {
                ...
                if (spItemsPresenter)
                {
                    ctl::ComPtr<ContentControl> spContentContainer;
                    BOOLEAN isHeader = (candidateElementType == Header);

                    if (isHeader)
                    {
                        IFC_RETURN(spItemsPresenter.Cast<ItemsPresenter>()->get_HeaderContainer(&spContentContainer));
                    }
                    else
                    {
                        IFC_RETURN(spItemsPresenter.Cast<ItemsPresenter>()->LoadFooter(TRUE));
                        IFC_RETURN(spItemsPresenter.Cast<ItemsPresenter>()->get_FooterContainer(&spContentContainer));
                    }

                    if (spContentContainer)
                    {
                        BOOLEAN isSelectable = FALSE;

                        IFC_RETURN(Selector::IsSelectableHelper(
                            ctl::iinspectable_cast(spContentContainer.Get()),
                            isSelectable));
                        ...
                        if (isSelectable)
                        {
                            ...
                            IFC_RETURN(spContentContainer.As(&spNextElement));
                        }
                    }
                }
            ...
            if (navigationMode == xaml_input::KeyboardNavigationMode::KeyboardNavigationMode_Local ||
                navigationMode == xaml_input::KeyboardNavigationMode::KeyboardNavigationMode_Cycle)
            {
                // Per spec: for local/cycle navigation mode iterate over all elements disregarding last focused element.
                ...
                else
                {
                    if (itemCount > 0)
                    {
                        m_lastFocusedIndex = itemCount - 1;
                    }
            ...
            else
            {
                ...
                if (initialElementType != ItemOrGroupHeader)
                {
                    ...
                    if (m_lastFocusedElementType != ElementType::Other)
                    {
                        ...
                        newFocusIndex = (isGroupHeader ? m_lastFocusedGroupIndex : m_lastFocusedIndex);
...
{
    HRESULT hr = S_OK;
    const bool isTabNavigationFullySupported = IsTabNavigationFullySupported();
    ...
    *pHandled = FALSE;
    ...
    if ((!isTabNavigationFullySupported &&
         shouldCustomizeTabNavigation) ||
        (isTabNavigationFullySupported &&
         (isUsingModernPanel ||                                                                                                                                   // #1
    ...
    {
        // When isTabNavigationFullySupported is True, this custom handling is not only needed for modern panels in general (#1), but also with a VirtualizingStackPanel or StackPanel:
        ...
        if (doProcessTabStopOverride)
        {
            ...
            if (isUsingModernPanel && !m_wasItemOrHeaderFocussed)
            {
            ...
            // Only provide tab stop suggestion if there is one and it's inside ListView.
                ...
                (candidateParentType != Other && spCandidateParent))
            {
                ...
                *pHandled = TRUE;
                m_wasItemOrHeaderFocussed = true;
            }
        }
    }
...
ListViewBase::OnGettingFocus(_In_ xaml::IUIElement* sender, _In_ xaml_input::IGettingFocusEventArgs* args)
{
    ...
    else if (IsTabNavigationFullySupported() &&
             (focusNavigationDirection == xaml_input::FocusNavigationDirection_Next || focusNavigationDirection == xaml_input::FocusNavigationDirection_Previous))
    {
        selectedIndex = m_lastFocusedIndex;
    }
    ...
    if (selectedIndex != -1)
    {
        ...
        if ((isFocusComingFromOutside || isFocusComingFromHeaderOrFooter) && isFocusMovingOntoItemOrGroupHeader)
        {
            ...
            IFC_RETURN(IsItemSelectable(itemMapping.Get(), selectedIndex, &isSelectable));
            if (isSelectable)
            {
                // Make sure the focused index is clear so that when we scroll to the selected item
                ...
                // error.
                // This will get set back to the selected item index when it receives focus, as a result
                ...
                ctl::ComPtr<xaml::IDependencyObject> selectedContainer;
                IFC_RETURN(ContainerFromIndex(selectedIndex, &selectedContainer));

                BOOLEAN focusMoved = false;
                IFC_RETURN(static_cast<GettingFocusEventArgs*>(args)->TrySetNewFocusedElement(selectedContainer.Get(), &focusMoved));
