...
_Check_return_ HRESULT CalendarView::OnSelectDayItem(_In_ CalendarViewDayItem* pItem)
{
    HRESULT hr = S_OK;
...
{
    auto lessThanComparer = m_dateComparer->GetLessThanComparer();
    ...
    auto pSelectedDates = m_tpSelectedDates.Cast<TrackableDateCollection>();
    ...
    if (!removedDates.empty())
    {
        ...
        {
            wf::DateTime date{};
            std::pair<TrackableDateCollection::DateSetType::iterator, TrackableDateCollection::DateSetType::iterator> result;
        ...
        for (auto it = removedDates.begin(); it != removedDates.end(); ++it)
        {
            ctl::ComPtr<CalendarViewDayItem> spChild;
    ...
    if (!addedDates.empty() || !removedDates.empty())
    {
        ...
        for (auto it = addedDates.begin(); it != addedDates.end(); ++it)
        {
...
_Check_return_ HRESULT CalendarView::OnDayItemBlackoutChanged(_In_ CalendarViewDayItem* pItem, _In_ bool isBlackOut)
{
    HRESULT hr = S_OK;
...
_Check_return_ HRESULT CalendarView::IsSelected(_In_ wf::DateTime date, _Out_ bool *pIsSelected)
{
    HRESULT hr = S_OK;
    unsigned index = 0;
    boolean found = false;
...
_Check_return_ HRESULT CalendarView::ValidateSelectingDateIsNotBlackout(_In_ wf::DateTime date)
{
    ctl::ComPtr<CalendarViewDayItem> spChild;
