...
_Check_return_ HRESULT ModernCollectionBasePanel::CorrectForEstimationErrors(bool adjustWindow /*= true*/)
{
    ...
    else if (m_containerManager.GetValidContainerCount() > 0)
    {
        ...
        // If we are preloading items 0 and 1, there's a chance that our first realized item could overlap with the preloaded items.
        // Since this doesn't involve hitting pixel 0 on the panel, this wouldn't be seen as a "regular" estimation error
        // So, let's detect this case and correct for it.
        if (m_enableCarouselPreload && realizedItemIndex > 1 && DoubleUtil::IsZero(neededCorrection))
        {
            INT preloadedItemIndex = -1;
            ctl::ComPtr<IUIElement> spPreloadedContainer;
            IFC(m_containerManager.GetElementFromPinnedElements(xaml_controls::ElementType_ItemContainer, 1, &spPreloadedContainer));
            if (spPreloadedContainer)
            {
                preloadedItemIndex = 1;
            }
            else
            {
                IFC(m_containerManager.GetElementFromPinnedElements(xaml_controls::ElementType_ItemContainer, 0, &spPreloadedContainer));
                if (spPreloadedContainer)
                {
                    preloadedItemIndex = 0;
                }
            }

            if (spPreloadedContainer)
            {
                ASSERT(preloadedItemIndex != -1);
                const wf::Rect preloadedBounds = GetBoundsFromElement(spPreloadedContainer);

                if (preloadedBounds.*PointFromRectInVirtualizingDirection() + preloadedBounds.*SizeFromRectInVirtualizingDirection() > firstContainerRect.*PointFromRectInVirtualizingDirection())
                {
                    wf::Rect desiredBounds = {};
                    EstimationReference headerRef = CreateDefaultEstimationReference(xaml_controls::ElementType_GroupHeader);
                    EstimationReference containerRef = CreateDefaultEstimationReference(xaml_controls::ElementType_ItemContainer);
                    containerRef.ElementBounds = preloadedBounds;
                    containerRef.ElementIndex = preloadedItemIndex;
                    IFC(m_spLayoutStrategy->EstimateElementBounds(
                        xaml_controls::ElementType_ItemContainer,
                        realizedItemIndex,
                        headerRef,
                    ...
                    neededCorrection = desiredBounds.*PointFromRectInVirtualizingDirection() - firstContainerRect.*PointFromRectInVirtualizingDirection();
                }
            }
        }
    }
