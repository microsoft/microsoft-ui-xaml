...
_Check_return_ HRESULT DesktopWindowXamlSource::Initialize()
{
    ...
    // when the GC does garbage collection. There's a m_owner WeakRef pointer on DirectUI::XamlIsland that points to this
    ...
    IFC_RETURN(frameworkApplication->CreateIslandWithContentBridge(ctl::iinspectable_cast(this), nullptr, m_spXamlIsland.ReleaseAndGetAddressOf()));
    ...
    ctl::ComPtr<XamlIsland> xamlIsland = m_spXamlIsland.Cast<XamlIsland>();
...
_Check_return_ xaml_hosting::IXamlIsland* DesktopWindowXamlSource::GetXamlIslandNoRef()
{
...
_Check_return_ HRESULT DesktopWindowXamlSource::ConnectToHwndIslandSite(_In_ HWND parentHwnd)
{
    ...
    else
    {
        IFC_RETURN(ctl::GetActivationFactory(wrl_wrappers::HStringReference(
            RuntimeClass_Microsoft_UI_Content_DesktopChildSiteBridge).Get(),
            &bridgeStatics));
    }
    ...
    // ScrollViewer.
    // Use the IDesktopChildSiteBridge's hwnd for now. The real one should come from the
    // input site.
    // Bug 24199635: Sort out input HWND with lifted IXP islands
    pXamlIslandCore->SetInputWindow(m_childHwnd);

    // Get the InputSite's HWND
    // Bug 24199635: Sort out input HWND with lifted IXP islands
//    UINT64 windowId;
//    wrl::ComPtr<IInputSiteForIslandPartnerInterop> inputSiteInterop;
//    IFCFAILFAST(inputSite.As(&inputSiteInterop));     // E_NOINTERFACE
//    IFC_RETURN(inputSiteInterop->GetWindowId(&windowId));

//    pXamlIslandCore->SetInputWindow(HWNDFromWindowId(windowId));
