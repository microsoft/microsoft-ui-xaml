...
_Check_return_ HRESULT DirectUI::ManipulationDeltaRoutedEventArgs::get_DeltaImpl(
    _Out_ ixp::ManipulationDelta* pValue)
{
    HRESULT hr = S_OK;
    CManipulationDelta* pDelta = nullptr;

    CEventArgs* const pCoreArgs = GetCorePeer();
    IFC(static_cast<CManipulationDeltaEventArgs*>(pCoreArgs)->GetDelta(&pDelta));
    if (pDelta)
    {
        pValue->Translation.X = pDelta->m_ptTranslation.x;
        pValue->Translation.Y = pDelta->m_ptTranslation.y;
        pValue->Scale = pDelta->m_fScale;
        pValue->Rotation = pDelta->m_fRotation;
        pValue->Expansion = pDelta->m_fExpansion;
    }
    else
    {
...
_Check_return_ HRESULT DirectUI::ManipulationDeltaRoutedEventArgs::get_CumulativeImpl(
    _Out_ ixp::ManipulationDelta* pValue)
{
    HRESULT hr = S_OK;
    ...
    CEventArgs* const pCoreArgs = GetCorePeer();
    IFC(static_cast<CManipulationDeltaEventArgs*>(pCoreArgs)->GetCumulative(&pCumulative));
    if (pCumulative)
    {
...
_Check_return_ HRESULT DirectUI::ManipulationDeltaRoutedEventArgs::get_VelocitiesImpl(
    _Out_ ixp::ManipulationVelocities* pValue)
{
    ...
    IFC_RETURN(static_cast<CManipulationDeltaEventArgs*>(coreArgs.get())->GetVelocities(velocities.ReleaseAndGetAddressOf()));
