    ...
    _Outptr_ IInspectable** children)
{
    ...
    {
        auto coreIsland = checked_cast<CXamlIslandRoot>(static_cast<DirectUI::XamlIsland*>(island)->GetHandle());
        auto collection = CDependencyObjectCollection::MakeDiagnosticsRootCollection(GetCore());
...
{
    return static_cast<DirectUI::XamlIsland*>(island)->TryGetOwner(islandOwner);
}
...
When we don't have a parent, it means a root is entering the tree, and we need to be careful about the roots that we hand back to VS.
    ...
    3. In XamlIslands, they are associated with the DesktopWindowXamlSource, or there are ways that internal customers can create XamlIslands today,
       and so we'll fallback to the XamlIsland if that's the case.
    ...
    _COM_Outptr_result_maybenull_ IInspectable** parent)
{
    *parent = nullptr;
    ...
    if (auto childElement = childItem.AsOrNull<xaml::IUIElement>())
    {
        ...
        // No direct parent, since VisualTreeHelper::GetParent won't return the XamlIsland, see if this element is supposed to be under a xaml island.
        ...
        if (auto xamlIslandRoot = VisualTree::GetXamlIslandRootForElement(coreChild))
        {
            ...
            {
                wrl::ComPtr<xaml_hosting::IXamlIsland> islandPeer;
                if (SUCCEEDED(DXamlCore::GetCurrent()->TryGetPeer<xaml_hosting::IXamlIsland>(xamlIslandRoot, islandPeer.ReleaseAndGetAddressOf())) && islandPeer)
                {
    ...
    _Outptr_ xaml::IXamlRoot** xamlRoot)
{
    ...
    if (auto xamlSource = spObject.AsOrNull<DirectUI::DesktopWindowXamlSource>())
    {
        ...
        auto coreIsland = checked_cast<CXamlIslandRoot>(static_cast<DirectUI::XamlIsland*>(xamlSource->GetXamlIslandNoRef())->GetHandle());
...
HRESULT DiagnosticsInterop::GetXamlIslandRoots(std::vector<wrl::ComPtr<IInspectable>>& roots)
{
    ...
    if (rootCollection)
    {
        ...
        {
            wrl::ComPtr<xaml_hosting::IXamlIsland> xamlIsland;
            if (SUCCEEDED(DXamlCore::GetCurrent()->TryGetPeer(child, xamlIsland.ReleaseAndGetAddressOf())) && xamlIsland)
            {
                wrl::ComPtr<IInspectable> islandOwner;
                if (TryGetOwnerFromIsland(xamlIsland.Get(), &islandOwner))
                {
                    roots.push_back(islandOwner);
                }
                else
                {
                    roots.push_back(xamlIsland);
                }
            }
        }
    }
