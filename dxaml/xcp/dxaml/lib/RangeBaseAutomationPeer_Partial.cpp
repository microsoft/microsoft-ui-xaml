...
RangeBaseAutomationPeer::RangeBaseAutomationPeer() :
    ...
    m_currentValuePropertiesReported(false),
    m_currentOldValueProperty(0),
    m_currentNewValueProperty(0)
{
}
...
_Check_return_ HRESULT RangeBaseAutomationPeer::ForceRaiseValuePropertyChangedEvent()
{
    // Due to event throttling in MediaTransportControls, the latest values may not have had an event raised for them.
    // Force report the values if this is the case.
    if (!m_currentValuePropertiesReported)
    {
        m_timePointOfLastValuePropertyChangedEvent = Jupiter::HighResolutionClock::now();

        CValue oldValue, newValue;
        oldValue.SetDouble(m_currentOldValueProperty);
        newValue.SetDouble(m_currentNewValueProperty);

        IFC_RETURN(ReportValuePropertyChangedEvent(oldValue, newValue));
    }
    ...
    _In_ const CValue& newValue)
{
    ...
    if (m_isEnableValueChangedEventThrottling)
    {
        ...
        // This flag is enabled only in MediaTransportControls for timer based change events while playing video.
        const unsigned int timeOutInMilliseconds = 8000;
    ...
    IFC_RETURN(CValueBoxer::UnboxValue(&oldValue, &m_currentOldValueProperty));
    IFC_RETURN(CValueBoxer::UnboxValue(&newValue, &m_currentNewValueProperty));
    m_currentValuePropertiesReported = false;
...
_Check_return_ HRESULT RangeBaseAutomationPeer::ReportValuePropertyChangedEvent(
    _In_ const CValue& oldValue,
    _In_ const CValue& newValue)
{
    ...
    m_currentValuePropertiesReported = true;
