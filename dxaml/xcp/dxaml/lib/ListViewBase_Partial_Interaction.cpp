    ...
    if (IsTabNavigationFullySupported())
    {
        ...
        if (navigationMode != xaml_input::KeyboardNavigationMode::KeyboardNavigationMode_Once)
        {
            ...
            if (singleSelectionFollowsFocus)
            {
                ...
                {
                    IFC(MakeSingleSelection(m_lastFocusedIndex, FALSE /*animateIfBringIntoView*/, nullptr /*pSelectedItem*/));
                }
            }
        }
    }
    ...
    if (gamepadDirection != xaml_input::FocusNavigationDirection::FocusNavigationDirection_None)
    {
        ...
        if (spCandidate)
        {
            ...
            if (isCandidateNotOwnListItem || (!spCandidateAsLVBI && !spCandidate.AsOrNull<ListViewBaseHeaderItem>()))
            {
                ...
                // walk up the tree until the listview to see if the candidate is within an item
                ...
                while (spCurrentItem)
                {
                    ...
                    if (ctl::are_equal(spCurrentItemAsI.Get(), header.Get()) || ctl::are_equal(spCurrentItemAsI.Get(), footer.Get()))
                    {
                        ...
                        // since the ScrollViewer within the listview will not handle scrolling, listview
        ...
        else
        {
            ...
            if (groupHeader)
            {
                ...
                if (isHandled && !wasGroupInitiallyFocused)
                {
                    ...
                    m_itemIndexHintForHeaderNavigation = m_lastFocusedIndex;
                }
            }
            else
            {
...
// Handles keyboard navigation for the ListView.  By default this will refer to
// the ItemsHost Panel to provide layout specific knowledge of keyboard
// navigation, but also includes generic StackPanel-like behavior if ListView is
    ...
    _Out_ xaml_controls::ElementType* pNewFocusedType)
{
    ...
    *pNewFocusedType = xaml_controls::ElementType_ItemContainer;
    ...
    else
    {
        ...
        IFC(GetGroupFromItem(m_lastFocusedIndex, &spCVGroup, &spSourceGroupItem, &sourceGroupIndex, &sourceIndexWithinGroup));
    ...
    if (spTargetPanel)
    {
        ...
        else
        {
            ...
            {
                UINT panelStartIndexInCollection = (isQueryingOuterPanel) ? 0 : m_lastFocusedIndex - sourceIndexWithinGroup;
                UINT startIndex = (isQueryingOuterPanel) ? m_lastFocusedIndex : sourceIndexWithinGroup;
        ...
        if (!isQueryingOuterPanel && isHandled)
        {
            ...
            if (newFocusedType == xaml_controls::ElementType_ItemContainer)
            {
                ...
                newFocusedIndex = m_lastFocusedIndex + indexDelta;
            }
        }
        ...
        if (useFallback)
        {
            if (!considerNestedPanels || doForceUseOuterPanel)
            {
                ...
                IFC(QuerySelectorFallbackForItemNavigation(key, m_lastFocusedIndex, &newFocusedIndex, &isHandled));
            }
            else
            {
    ...
    else if (allowPanelWrap && !isQueryingOuterPanel && isNavDirectionParallelToInnerOrientation)
    {
        ...
        BOOLEAN doIncreaseIndex = (action == xaml_controls::KeyNavigationAction_Right) || (action == xaml_controls::KeyNavigationAction_Down);
        if (doIncreaseIndex)
        {
            ctl::ComPtr<wfc::IObservableVector<IInspectable*>> spItems;
            UINT itemCount = 0;
            ...
            if (m_lastFocusedIndex <= itemCount - 2)
            {
                *pNewFocusedIndex = m_lastFocusedIndex + 1;
            }
        }
        else if (m_lastFocusedIndex > 0)
        {
            *pNewFocusedIndex = m_lastFocusedIndex - 1;
        }
    }
    ...
    _In_ BOOLEAN hasFocus)
{
    ...
    if (hasFocus)
    {
        ...
        {
            targetFocusedItem = static_cast<INT>(m_lastFocusedIndex);
        }
    ...
    _In_ ListViewBaseItem* pItem)
{
    ...
    // if ListView loses and regains focus
    if (index >= 0)
    {
        m_lastFocusedIndex = static_cast<UINT>(index);
    }
