...
LRESULT CALLBACK CJupiterWindow::CoreWindowSubclassProc(_In_ HWND hwnd, UINT uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_SETTINGCHANGE:
            OnCoreWindowSettingChanged(lParam);
            fDefaultToControl = false;
            break;
...
XSIZE CJupiterWindow::GetJupiterWindowPhysicalSize(HWND hwndContext) const
{
    if (XamlOneCoreTransforms::IsEnabled())
    {
        ...
        const auto mainRoot = rootCoordinator->Unsafe_XamlIslandsIncompatible_CoreWindowContentRoot();
...
void CJupiterWindow::OnCoreWindowSettingChanged(_In_ LPARAM lParam)
{
    if (m_pControl)
    {
        VERIFYHR(m_pControl->HandleSettingChangedMessage(lParam));
    }
}
...
CContentRoot* CJupiterWindow::GetCoreWindowContentRootNoRef() const
{
    ...
    return contentRootCoordinator->Unsafe_XamlIslandsIncompatible_CoreWindowContentRoot();
}
