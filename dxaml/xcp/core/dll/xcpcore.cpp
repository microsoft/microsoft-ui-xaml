...
ixp::IInputFocusControllerStatics* ActivationFactoryCache::GetInputFocusControllerStatics()
{
    wil::cs_leave_scope_exit guard = m_lock.lock();
...
_Check_return_ HRESULT CCoreServices::ResetState()
{
    ...
    if (CContentRoot* contentRoot = m_contentRootCoordinator.Unsafe_IslandsIncompatible_CoreWindowContentRoot())
    {
...
CCoreServices::UIAClientsAreListening(_In_ UIAXcp::APAutomationEvents eAutomationEvent)
{
    ...
    if (m_pBrowserHost != nullptr)
    {
        ...
        if (HasXamlIslandRoots())
        {
            // Check to see if we have a XamlIslandRoot that has active UIA
    ...
    )
{
    ...
    *pFrameDrawn = false;
    ...
    if (!m_pHostSite->IsWindowDestroyed())
    {
        ...
        if( isRenderEnabled && m_framesToSkip != 0)
        {
            m_framesToSkip--;
        ...
        else if (isRenderEnabled)
        {
            ...
            if (needsRenderWalk)
            {
                ...
                // Ensure XamlIslandRoot target roots have been created
    ...
    _Out_ bool* canSubmitFrame)
{
    ...
    *canSubmitFrame = true;
    ...
    //
    // Note that we can't do this cleanup as part of RenderRoot, because in the case of XamlIslandRoots we'll call RenderRoot
    ...
    float scale = 1.0f;
    if (const auto root = GetContentRootCoordinator()->Unsafe_IslandsIncompatible_CoreWindowContentRoot())
    {
    ...
    TraceRenderWalkEnd1(pHWWalk->GetElementsVisited(), pHWWalk->GetElementsRenderedCount());
    ...
    _In_ CTimeManager *pTimeManager)
{
    ...
    // Note that the composition trees for each XamlIslandRoot are not connected
...
_Check_return_ HRESULT CCoreServices::IsObjectAnActivePopupRoot(_In_ CDependencyObject *pObject, _Out_ bool *pIsActivePopupRoot)
{
    ...
    if (!isActivePopupRoot && HasXamlIslandRoots())
    {
        CXamlIslandRootCollection* xamlIslandRootCollection = m_pMainVisualTree->GetXamlIslandRootCollection();
        CDOCollection* collection = xamlIslandRootCollection->GetChildren();
...
_Check_return_ HRESULT CCoreServices::OnVisibilityChanged(bool isStartingUp, bool freezeDWMSnapshotIfHidden)
{
    ...
    else
    {
        ...
        if (GetInitializationType() != InitializationType::IslandsOnly)
        {
            if (CContentRoot* coreWindowContentRoot = m_contentRootCoordinator.Unsafe_IslandsIncompatible_CoreWindowContentRoot())
            {
    ...
    float testOverrideScale)
{
    ...
    // Save the scale override so we can set it on any islands created after this call.
    m_testOverrideScale = testOverrideScale;
...
void CCoreServices::RemoveXamlIslandRoot(_In_ CXamlIslandRoot* xamlIslandRoot)
{
    ...
    void* result = xamlIslandRoots->Remove(xamlIslandRoot);
...
bool CCoreServices::HasXamlIslandRoots() const
{
    return m_pNWWindowRenderTarget
