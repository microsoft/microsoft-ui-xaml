    ...
    if (pCore->GetDeployment())
    {
        if (pCore->GetDeployment()->m_pApplication)
        {
            ...
            if (pPublicRootVisual)
            {
                ...
                pPublicRootVisual->SetVisualTree(pCore->GetContentRootCoordinator()->Unsafe_XamlIslandsIncompatible_CoreWindowContentRoot()->GetVisualTreeNoRef());
            }
    ...
    _In_opt_ CContentPresenter* pContentPresenter)
{
    ...
    if (pVisualRoot)
    {
        CContentRoot* contentRoot = pCore->GetContentRootCoordinator()->Unsafe_XamlIslandsIncompatible_CoreWindowContentRoot();
    ...
    _In_opt_ CUIElement* pTransitionElement)
{
    ...
    {
        // It's possible pTargetUIElement was in a XamlIsland, but is no longer parented.  We'll check
        // both the pTargetUIElement and pTransitionElement to see if they were in an island.
        CXamlIslandRoot* xamlIsland = VisualTree::GetXamlIslandRootForElement(pTargetUIElement);
        if (!xamlIsland)
        {
            xamlIsland = VisualTree::GetXamlIslandRootForElement(pTransitionElement);
        }

        if (xamlIsland)
        {
            layerNoRef = xamlIsland->GetTransitionRootNoRef();
        }
        else
        {
    ...
    _Outptr_ CGeneralTransform** returnValue)
{
    ...
    if (pRelativeElement && pCore->HasXamlIslands())
    {
