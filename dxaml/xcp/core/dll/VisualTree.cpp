...
_Check_return_ HRESULT VisualTree::Shutdown()
{
    ...
    // For XamlIslandRoots, this object has a reference on the composition island, which keeps a MockDComp island alive
    // during test cleanup.
    if (m_rootScale)
    {
        m_rootScale.reset();
    ...
    , m_coreContentRoot(coreContentRoot)
{
    ...
    else if (m_coreContentRoot.GetType() == CContentRoot::Type::XamlIslandRoot)
    {
        ...
        const float testOverrideScale = m_pCoreNoRef->GetTestOverrideScale();
    ...
    _Outptr_result_maybenull_ CPopupRoot **ppPopupRoot)
{
    ...
    if (core->HasXamlIslandRoots() && core->GetInitializationType() == InitializationType::IslandsOnly)
    {
...
{
    if (!pObject || !pObject->GetContext()->HasXamlIslandRoots())
    {
        return nullptr;
    }
    if (VisualTree* visualTree = GetForElementNoRef(pObject))
...
{
    if (element->GetTypeIndex() == KnownTypeIndex::XamlIslandRoot)
    {
        return;
    }
...
void VisualTree::ClearTestLTEs()
{
    ...
    {
        // Absolutely positioned LTEs may have been removed from the tree already if their target leaves the tree.
        // Don't try removing them again.
        if (lte->GetParent() != nullptr)
        {
...
wf::Size VisualTree::GetSize() const
{
    ...
    else if (CXamlIslandRoot* xamlIslandRoot = do_pointer_cast<CXamlIslandRoot>(m_rootElement))
    {
...
POINT VisualTree::GetScreenOffset() const
{
    ...
    if (CXamlIslandRoot* xamlIslandRoot = do_pointer_cast<CXamlIslandRoot>(m_rootElement))
    {
        return xamlIslandRoot->GetScreenOffset();
    }
    else if (CRootVisual* rootVisual = do_pointer_cast<CRootVisual>(m_rootElement))
    {
