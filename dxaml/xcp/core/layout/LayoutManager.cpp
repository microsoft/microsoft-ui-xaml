...
CLayoutManager::UpdateLayout(XUINT32 controlWidth, XUINT32 controlHeight)
{
    ...
    bool previousCoreIsLayoutCycleTrackingActive = false;
    ...
    while (count--)
    {
        ASSERT(!m_isLayoutCycleLoggingSuspended); // suspend should be short. Each iteration should not start suspended
        ...
        if (count < WarningLayoutIterations)
        {
            ...
            m_elementsIgnoreDesiredSizeChanged.clear(); // stop ignoring anything from the previous iteration

            // If this is the first Warning iteration, update the state on the core.
            if (count == WarningLayoutIterations-1)
            {
                previousCoreIsLayoutCycleTrackingActive = m_pCoreServices->IsLayoutCycleTrackingActive();
                m_pCoreServices->SetIsLayoutCycleTrackingActive(true);
            }
        }
    ...
    ASSERT(!m_isLayoutCycleLoggingSuspended); // suspend should be short, shouldn't still be suspended
    if (StoreLayoutCycleWarningContexts())
    {
        m_layoutCycleWarningContextsCountdown = -1;
        m_pCoreServices->SetIsLayoutCycleTrackingActive(previousCoreIsLayoutCycleTrackingActive);
...
bool CLayoutManager::ShouldReportDesiredSizeChanged(_In_ CUIElement* element)
{
    if (StoreLayoutCycleWarningContexts())
    {
        auto itFind = std::find(m_elementsIgnoreDesiredSizeChanged.begin(), m_elementsIgnoreDesiredSizeChanged.end(), element);
        if (itFind == m_elementsIgnoreDesiredSizeChanged.end())
        {
            // element isn't in the ignore list so it should report its DesiredSize
            // change. Add all ancestors to the ignore list.
            CUIElement *parent = do_pointer_cast<CUIElement>(element->GetParentInternal(false));
            while (parent)
            {
                m_elementsIgnoreDesiredSizeChanged.push_back(parent);
                parent = do_pointer_cast<CUIElement>(parent->GetParentInternal(false));
            }
            return true;
        }
    }
