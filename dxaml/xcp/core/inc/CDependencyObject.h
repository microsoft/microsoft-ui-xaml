...
namespace ctl
{
    ...
    class interface_forwarder<xaml::Core::Direct::IXamlDirectObject, impl_type> : public interface_forwarder_base<xaml::Core::Direct::IXamlDirectObject>
    {
            ...
            /* [iid_is][out] */ _COM_Outptr_ void __RPC_FAR *__RPC_FAR *ppvObject) final
        {
            if (InlineIsEqualGUID(riid, __uuidof(IUnknown))
                || InlineIsEqualGUID(riid, __uuidof(IInspectable))
                || InlineIsEqualGUID(riid, __uuidof(xaml::Core::Direct::IXamlDirectObject)))
            {
                *ppvObject = ctl::interface_cast<xaml::Core::Direct::IXamlDirectObject>(This());
            }
            else
            {
                *ppvObject = nullptr;
        ...
        {
            ULONG cRefCount = This()->GetRefCount();
            This()->Release();
            return cRefCount - 1;
        }
        ...
        {
            static const IID* const cdoIIDs[] = {
                &__uuidof(IUnknown),
                &__uuidof(IInspectable),
                &__uuidof(xaml::Core::Direct::IXamlDirectObject),
            };

            IID* iidArray = static_cast<IID*>(::CoTaskMemAlloc(sizeof(IID) * ARRAY_SIZE(cdoIIDs)));
            for (unsigned int i = 0; i < ARRAY_SIZE(cdoIIDs); ++i)
            {
                iidArray[i] = *cdoIIDs[i];
            }

            *iidCount = ARRAY_SIZE(cdoIIDs);
            *iids = iidArray;
            return S_OK;
        }
        ...
        {
            constexpr wchar_t classNameStr[] = L"Microsoft.UI.Xaml.IXamlDirectObject";
...
class CDependencyObject : public ctl::forwarder_holder<xaml::Core::Direct::IXamlDirectObject, CDependencyObject>
{
