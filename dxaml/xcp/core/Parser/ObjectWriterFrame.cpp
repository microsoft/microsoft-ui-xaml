...
{
    auto it = std::find_if(m_assignedProperties.m_vector.begin(), m_assignedProperties.m_vector.end(),
        [&inProperty](const std::shared_ptr<XamlProperty>& value) -> bool
    {
        return !!XamlProperty::AreEqual(inProperty, value);
    });
    return it != m_assignedProperties.m_vector.end();
}
    ...
    _In_ const std::shared_ptr<XamlProperty>& inProperty)
{
    ...
    m_assignedProperties.m_vector.push_back(inProperty);
}
