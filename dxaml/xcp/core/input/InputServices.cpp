...
CInputServices::UpdateCursor(_In_ CDependencyObject* pVisualInTargetIsland, _In_ XINT32 bUnset)
{
    ...
    // If XamlIsland does not exist, set InputPointerSource.Cursor on JupiterWindow's InputSiteAdapter
...
// Intended to be called in XamlOneCoreTransforms mode or when using XamlIslands.
_Check_return_ HRESULT
    ...
    _Out_ PointerInfo* pointerInfoResult)
{
    ...
    // Since XamlIslands use CoreComponentInput, we keep them on the OneCore input path
    // as well.
...
void CInputServices::SetCoreWindow(_In_ wuc::ICoreWindow* pCoreWindow)
{
    ...
    CContentRoot* contentRoot = m_pCoreService->GetContentRootCoordinator()->Unsafe_XamlIslandsIncompatible_CoreWindowContentRoot();
