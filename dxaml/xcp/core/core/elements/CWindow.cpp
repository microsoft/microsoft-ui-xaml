...
_Check_return_ HRESULT CWindow::SetValue(_In_ const SetValueParams& args)
{
    ...
    {
    // Window_Content is a parser-only private property; hand it to the DXaml Window peer and don't store the core DP, so the markup-loaded root can be released when the app clears or replaces Window.Content.
    case KnownPropertyIndex::Window_Content:
        {
            ...
            if (!args.m_value.IsNullOrUnset())
            {
                ...
                IFC_RETURN(DirectUI::DXamlCore::GetCurrent()->GetPeer<DirectUI::UIElement>(args.m_value.AsObject(), &uielement));
