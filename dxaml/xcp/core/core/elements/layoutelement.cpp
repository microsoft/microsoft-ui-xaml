...
CLayoutElement::MeasureCore(XSIZEF availableSize, _Out_ XSIZEF& desiredSize)
{
    ...
    XSIZEF frameworkAvailableSize = {
        IsFiniteF(availableSize.width) ? std::max(availableSize.width - marginWidth, 0.0f) : XFLOAT_INF,
        IsFiniteF(availableSize.height) ? std::max(availableSize.height - marginHeight, 0.0f) : XFLOAT_INF
    };
