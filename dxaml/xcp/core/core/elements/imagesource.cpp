    ...
    )
{
    ...
    bitmapSource->SetupImageSource(true /*mustKeepSoftwareSurface*/);
...
CImageSource::SetValue(_In_ const SetValueParams& args)
{
    ...
    else if (args.m_pDP->GetIndex() == KnownPropertyIndex::BitmapImage_CreateOptions)
    {
        ...
        // already in progress.
        //
        if (!m_pImageSurfaceWrapper->HasAnySurface() && m_spAbortableImageOperation == NULL)
        {
...
_Check_return_ HRESULT CImageSource::OnImageViewUpdated(ImageViewBase& sender)
{
    ...
    if (ShouldContinueAsyncAction())
    {
        ...
        HRESULT parseHR = m_imageMetadataView->GetHR(GetContext()->GetImageProvider()->GetDecodeActivity(), reinterpret_cast<uint64_t>(this));
