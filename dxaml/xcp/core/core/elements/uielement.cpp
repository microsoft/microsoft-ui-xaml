    ...
    _Outptr_ ITransformer** returnValue)
{
    ...
    *returnValue = nullptr;
    ...
    // 19H1 Bug #19652900:  When the element is in a XamlIsland, we cannot rely on GetTreeRoot() logic as this
    // function is unaware of islands, and this ends up causing us to find the wrong root when pVisual == NULL.
    // The fix is to detect when we're in a XamlIsland scenario and retrieve the appropriate root from ContentRoot.
    CContentRoot* contentRoot = VisualTree::GetContentRootForElement(this);
    if (contentRoot != nullptr && contentRoot->GetType() == CContentRoot::Type::XamlIsland)
    {
...
CUIElement::InvalidateMeasure()
{
    if (!GetIsMeasureDirty() && !GetIsMeasuringSelf())
    {
        ...
        {
            StoreLayoutCycleWarningContext(VisualTree::GetLayoutManagerForElement(this));
...
//
//------------------------------------------------------------------------
float CUIElement::LayoutRoundHelper(const float value, const bool useCeiling)
{
    ...
    if (scaleFactor != 1.0)
    {
        returnValue = useCeiling
            ? (XcpCeiling(returnValue * scaleFactor) / scaleFactor)
            : (XcpRound(returnValue * scaleFactor) / scaleFactor);
    }
    else
    {
        // Avoid unnecessary multiply/divide at scale factor 1.
        returnValue = useCeiling
            ? XcpCeiling(returnValue)
            : XcpRound(returnValue);
    }
...
{
    return LayoutRoundHelper(value, false /* useCeiling */);
}
...
{
    return LayoutRoundHelper(value, true /* useCeiling */);
}
...
_Check_return_ XUINT32 CUIElement::GetAPPopupChildren(_In_ XUINT32 nCount, _Inout_updates_opt_(nCount) CAutomationPeer ** ppChildrenAP)
{
    ...
    if (pCollection && (ppChildrenAP == NULL || nCount > 0))
    {
        ...
        if (uAPCount > 0 && pPopupRoot->IsTopmostPopupInLightDismissChain() && !OfTypeByIndex<KnownTypeIndex::XamlIsland>())
        {
    ...
    _Out_ bool* isRootNull)
{
    if (coreServices->GetInitializationType() == InitializationType::IslandsOnly)
    {
        ...
        *ppThisNoRef = uiElement;
        ...
        if (uiElementAncestor != nullptr && uiElementAncestor->GetTypeIndex() == KnownTypeIndex::XamlIsland)
        {
    ...
    )
{
    ...
    if (isVisible)
    {
        ...
        if (areAncestorsVisible)
        {
            ...
            const bool isXamlIslandRoot = this->GetTypeIndex() == KnownTypeIndex::XamlIsland;
            if (isRootAndOneCoreStrict || isXamlIslandRoot)
            {
                ...
                // In XamlIslands mode, the XamlIslandRoot alone knows the correct scale.
    ...
    if (m_pLayoutTransitionRenderers != nullptr && !m_pLayoutTransitionRenderers->empty())
    {
        ...
        for (xvector<CLayoutTransitionElement*>::const_reverse_iterator it = m_pLayoutTransitionRenderers->rbegin(); it != rend; ++it)
        {
            if ((*it)->IsAbsolutelyPositioned())
            {
                ...
                CDependencyObject* pParent = pLTE->GetParentInternal(false);
                ASSERT(pParent->OfTypeByIndex<KnownTypeIndex::TransitionRoot>());
                CTransitionRoot* pLayer = static_cast<CTransitionRoot*>(pParent);
    ...
    )
{
    ...
    // This because the XamlIsland that owns the Xaml tree has been Disposed.
    ...
    )
{
    ...
    for (CUIElement* pElement = pDescendantElement;
        ...
        pElement = pElement->GetUIElementAdjustedParentInternal(false))
    {
        if (pElement->OfTypeByIndex<KnownTypeIndex::XamlIsland>())
        {
            // A XamlIsland knows its accumulated scale.  Once we have this scale, we just use it, no
