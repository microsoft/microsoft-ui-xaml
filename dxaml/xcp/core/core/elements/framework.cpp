...
CFrameworkElement::MeasureCore(XSIZEF availableSize, XSIZEF& desiredSize)
{
    ...
    XFLOAT marginWidth  = 0.0f;
    XFLOAT marginHeight = 0.0f;
    ...
    if (!bInLayoutTransition)
    {
        ...
        frameworkAvailableSize.width = IsFiniteF(availableSize.width) ? MAX(availableSize.width - marginWidth, 0) : XFLOAT_INF;
        frameworkAvailableSize.height = IsFiniteF(availableSize.height) ? MAX(availableSize.height - marginHeight, 0) : XFLOAT_INF;
    ...
    if (!bInLayoutTransition)
    {
        ...
        XFLOAT roundedMarginWidth = marginWidth;
        XFLOAT roundedMarginHeight = marginHeight;
        if (GetUseLayoutRounding())
        {
...
CFrameworkElement::ArrangeCore(XRECTF finalRect)
{
    ...
    XFLOAT marginWidth = m_pLayoutProperties->m_margin.left + m_pLayoutProperties->m_margin.right;
    XFLOAT marginHeight = m_pLayoutProperties->m_margin.top + m_pLayoutProperties->m_margin.bottom;
    ...
    if (!bInLayoutTransition)
    {
        ...
        arrangeSizeWithoutMargin.width = MAX(arrangeSize.width - marginWidth, 0);
        arrangeSizeWithoutMargin.height = MAX(arrangeSize.height - marginHeight, 0);
    ...
    if (!bInLayoutTransition)
    {
        ...
        clientSize.width = MAX(0, finalRect.Width - marginWidth);
        clientSize.height = MAX(0, finalRect.Height - marginHeight);
