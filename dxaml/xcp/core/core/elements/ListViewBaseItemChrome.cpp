...
CItemsControl* CListViewBaseItemChrome::GetParentItemsControl(_In_opt_ CContentControl* listViewBaseItem) const
{
    CDependencyObject* parent = listViewBaseItem;
    while (parent != nullptr && !parent->OfTypeByIndex<KnownTypeIndex::ItemsControl>())
    {
        parent = parent->GetParentFollowPopups();
    }
    return (parent != nullptr) ? static_cast<CItemsControl*>(parent) : nullptr;
}
...
CListViewBaseItemChrome::IsInSelectionIndicatorMode() const
{
    ...
    CItemsControl* itemsControlNoRef = GetParentItemsControl(m_pParentListViewBaseItemNoRef);

    if (!itemsControlNoRef)
    {
        return false;
    }
    ...
    IFCFAILFAST(itemsControlNoRef->GetValueByIndex(KnownPropertyIndex::ListViewBase_SelectionMode, &valSelectionMode));
