...
// All formats recognized by XAML and requested from TranslateColorGlyphRun when DWRITE_GLYPH_IMAGE_FORMATS_COLR_PAINT might be available.
auto constexpr c_allSupportedGlyphImageFormats8   = DWRITE_GLYPH_IMAGE_FORMATS_TRUETYPE
...
auto constexpr c_allSupportedGlyphImageFormats    = DWRITE_GLYPH_IMAGE_FORMATS_TRUETYPE
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_CFF
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_COLR
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_SVG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_PNG
...
auto constexpr c_supportedColorGlyphImageFormats  = DWRITE_GLYPH_IMAGE_FORMATS_COLR
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_COLR_PAINT_TREE
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_SVG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_PNG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_JPEG
...
auto constexpr c_bgraColorGlyphImageFormats       = DWRITE_GLYPH_IMAGE_FORMATS_COLR_PAINT_TREE
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_SVG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_PNG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_JPEG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_TIFF
...
auto constexpr c_anyBitmapGlyphImageFormats       = DWRITE_GLYPH_IMAGE_FORMATS_PNG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_JPEG
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_TIFF
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_PREMULTIPLIED_B8G8R8A8;

// Purely monochromatic formats which use the foreground brush.
auto constexpr c_monochromeGlyphImageFormats      = DWRITE_GLYPH_IMAGE_FORMATS_TRUETYPE
                                                  | DWRITE_GLYPH_IMAGE_FORMATS_CFF;
    ...
    )
{
    ...
    if (glyphImageFormat == GetDWriteGlyphImageFormats())
    {
...
// Determine and return the supported glyph formats.
DWRITE_GLYPH_IMAGE_FORMATS D2DTextDrawingContext::GetDWriteGlyphImageFormats()
{
    if (m_dwriteGlyphImageFormats == 0)
    {
        Microsoft::WRL::ComPtr<IDWriteFactory4> dWriteFactory4;
        IFCFAILFAST(m_pTextCore->GetWinTextCore()->GetDWriteFactory(&dWriteFactory4));

        Microsoft::WRL::ComPtr<IDWriteFactory8> dWriteFactory8;
        if (SUCCEEDED(dWriteFactory4.As(&dWriteFactory8)))
        {
            // IDWriteFactory8 is supported, so we can use all the formats supported by *8
            m_dwriteGlyphImageFormats = c_allSupportedGlyphImageFormats8;
        }
        else
        {
            m_dwriteGlyphImageFormats = c_allSupportedGlyphImageFormats;
        }
    }

    return m_dwriteGlyphImageFormats;
}
    ...
    )
{
    *hadColorGlyphs = FALSE;
    ...
    HRESULT hrTranslate;
    if (GetDWriteGlyphImageFormats() == c_allSupportedGlyphImageFormats8)
    {
        ...
        Microsoft::WRL::ComPtr<ID2D1DeviceContext7> deviceContext7;
        IFC_RETURN(pSharedD2DDeviceContextNoRef->QueryInterface(IID_PPV_ARGS(&deviceContext7)));

        Microsoft::WRL::ComPtr<IDWriteFactory8> dWriteFactory8;
        IFC_RETURN(dWriteFactory4.As(&dWriteFactory8));
        hrTranslate = dWriteFactory8->TranslateColorGlyphRun(
            { baselineOffset.x, baselineOffset.y },
            &dwriteGlyphRun,
            NULL,
            GetDWriteGlyphImageFormats(),
            deviceContext7->GetPaintFeatureLevel(),
    ...
    {
        hrTranslate = dWriteFactory4->TranslateColorGlyphRun(
            { baselineOffset.x, baselineOffset.y },
            &dwriteGlyphRun,
            NULL,
            GetDWriteGlyphImageFormats(),
...
{
    // FUTURE: When eventually we only support OS versions which all have ID2D1DeviceContext7
    //         we could replace this code with a call to DrawGlyphRunWithColorSupport().
    if (glyphImageFormat == DWRITE_GLYPH_IMAGE_FORMATS_COLR_PAINT_TREE)
    {
        Microsoft::WRL::ComPtr<ID2D1DeviceContext7> deviceContext7;
        IFCFAILFAST(pDeviceContext->QueryInterface(IID_PPV_ARGS(&deviceContext7)));
        deviceContext7->DrawPaintGlyphRun(
            D2D1::Point2F(),
            &glyphRun,
            pBrushNoRef,
            fontPaletteIndex,
    ...
    else if (glyphImageFormat == DWRITE_GLYPH_IMAGE_FORMATS_SVG)
    {
