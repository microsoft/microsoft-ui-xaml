...
// Safe cast from T** to CDependencyObject**. Verifies that T inherits from CDependencyObject
template<class T> XCP_FORCEINLINE
CDependencyObject** ido_pp_cast(_In_ T** pObject)
{
    DerivedFromConstraint<T, CDependencyObject>();
    return reinterpret_cast<CDependencyObject**>(pObject);
}

// Safe cast from T** to CDependencyObject**. Verifies that T inherits from CDependencyObject
template<class T> XCP_FORCEINLINE
CDependencyObject** do_pp_cast(_In_ T** pObject)
{
    DerivedFromConstraint<T, CDependencyObject>();
    return reinterpret_cast<CDependencyObject**>(pObject);
};
...
struct IMFMediaType;
_Check_return_ extern "C" HRESULT __stdcall //STDAPI
MFUnwrapMediaType(
