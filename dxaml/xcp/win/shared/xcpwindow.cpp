...
//
//------------------------------------------------------------------------
void CDeferredInvoke::RegisterTarget(CXcpDispatcher* dispatcher)
{
...
//
//  Synopsis:
//      Enqueues a deferred invoke request for the target's thread.  Can be called from any
//      thread.
//
//------------------------------------------------------------------------
...
void CDeferredInvoke::DispatchQueuedMessage(_Out_ bool* dispatchedWork, _Out_ bool* hasMoreWork)
{
...
CXcpDispatcher::Init(_In_ IXcpHostSite *pSite)
{
    ...
    m_DeferredInvoke.RegisterTarget(this);
...
//
//------------------------------------------------------------------------
void CXcpDispatcher::SendMessage(
    _In_ UINT uMsg,
    _In_ WPARAM wParam,
    _In_ LPARAM lParam)
{
    ASSERT(uMsg >= WM_FIRST && uMsg <= WM_LAST);
...
Cleanup:
    if (FAILED(hr))
    {
        ...
        ReleaseMessageResources(msg, lParam);
    }
}
...
//
//  Synopsis:
//      Posts a tick message.  Can be called from any thread.
//
//------------------------------------------------------------------------
_Check_return_ HRESULT
