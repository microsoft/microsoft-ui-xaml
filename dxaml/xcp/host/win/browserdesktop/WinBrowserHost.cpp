    ...
    _In_opt_ INTERNAL_EVENT_HANDLER pHandler)
{
    ...
    // Send message
    IFC_RETURN((static_cast<CXcpDispatcher *>(pDispatcher))->SendMessage(
                uMsg,
                0,
...
_Check_return_ HRESULT CXcpBrowserHost::OnPaint()
{
    ...
    if (m_forceRedrawOnPaint)
    {
        m_forceRedrawOnPaint = FALSE;

        // Request a tick to forcibly redraw the entire tree
        IFrameScheduler *pScheduler = GetFrameScheduler();
        if (pScheduler != NULL)
        {
            m_fForceRedraw = true;
            IFC_RETURN(pScheduler->RequestAdditionalFrame(0 /*immediate*/, RequestFrameReason::Paint));
        }
    }
    ...
    _In_opt_ XHANDLE hwnd)
{
    ...
    // XamlIslands: Moving/sizing the CoreWindow shouldn't affect XamlIslands-only scenarios
    ...
    if (presentTarget == nullptr || hwnd != presentTarget->GetTargetWindowHandle())
    {
        ...
        XHANDLE hDispatcherWindow = static_cast<CXcpDispatcher *>(m_pDispatcherNoRef)->GetHandle();
