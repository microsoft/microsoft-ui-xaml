    ...
    )
{
    ...
    // If the surface became smaller in either dimension, then we could have pending surface updates that update
    // a part of the surface that exceeds the new, smaller bounds. Clear out the pending update list.
    // Note: We could walk through the update list and clear out only those that reach outside the new bounds.
    // To keep things simple, we don't bother.
