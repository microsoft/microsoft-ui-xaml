...
CD3D11Device::~CD3D11Device()
{
    ...
    IGNOREHR(CleanupDXGIDeviceManager());
}
...
//
//  Synopsis:
//      Get DXGIDeviceManager associated with this device.
//
//------------------------------------------------------------------------------
_Check_return_ HRESULT
CD3D11Device::GetDXGIDeviceManager(_Outptr_ IMFDXGIDeviceManager **ppDeviceManager)
{
    HRESULT hr = S_OK;
    HMODULE hMFPlatModule = NULL;
    ...
    *ppDeviceManager = NULL;
    ...
    {
        MFLOCKDXGIDEVICEMANAGER pfnMFLockDXGIDeviceManager = NULL;
        HANDLE  pDeviceHandle = NULL;

        // Load mfplat.dll manually, to avoid runtime dependency on mfplat.dll, which is
        // not required for non-media scenario.
        IFCW32(hMFPlatModule = LoadLibraryEx(L"mfplat.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32));

        pfnMFLockDXGIDeviceManager =
                reinterpret_cast<MFLOCKDXGIDEVICEMANAGER>(
                    GetProcAddress(hMFPlatModule, "MFLockDXGIDeviceManager"));

        IFCPTR(pfnMFLockDXGIDeviceManager);

        //
        // Locks the shared Microsoft DirectX Graphics Infrastructure (DXGI) Device Manager,
        // This will be passed to MediaEngine, so that MediaEngine can detect the device lost and
        // reset it on DCOMP Mode.
        //
        // This DXGIDeviceManager will be unlocked later when it is no longer used.
        //
        IFC(pfnMFLockDXGIDeviceManager(&m_uResetToken, &m_pDXGIDeviceManager));

        hr = m_pDXGIDeviceManager->OpenDeviceHandle(&pDeviceHandle);

        if (hr == MF_E_DXGI_DEVICE_NOT_INITIALIZED)
        {
            CD3D11SharedDeviceGuard guard;
            IFC(m_deviceInstance->TakeLockAndCheckDeviceLost(&guard));
            IFC(m_pDXGIDeviceManager->ResetDevice(m_deviceInstance->GetDevice(&guard), m_uResetToken));
        }
        else
        {
            if (pDeviceHandle != NULL)
            {
                IFC(m_pDXGIDeviceManager->CloseDeviceHandle(pDeviceHandle));
            }
        }
     }

    SetInterface(*ppDeviceManager, m_pDXGIDeviceManager);

Cleanup:

    if (hMFPlatModule != NULL)
    {
        FreeLibrary(hMFPlatModule);
    }
...
//  Synopsis:
//
//      Cleanup DXGIDeviceManager when it is no longer required.
//
//------------------------------------------------------------------------------
_Check_return_  HRESULT
CD3D11Device::CleanupDXGIDeviceManager( )
{
    HRESULT hr = S_OK;
    HMODULE hMFPlatModule = NULL;
    ...
    if (m_pDXGIDeviceManager != NULL)
    {
        MFUNLOCKDXGIDEVICEMANAGER pfnMFUnlockDXGIDeviceManager = NULL;

        // Load mfplat.dll manually, to avoid runtime dependency on mfplat.dll, which is
        // not required for non-media scenario.
        IFCW32(hMFPlatModule = LoadLibraryEx(L"mfplat.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32));

        pfnMFUnlockDXGIDeviceManager =
                reinterpret_cast<MFUNLOCKDXGIDEVICEMANAGER>(
                    GetProcAddress(hMFPlatModule, "MFUnlockDXGIDeviceManager"));

        IFCPTR(pfnMFUnlockDXGIDeviceManager);

        // Unlocks the shared Microsoft DirectX Graphics Infrastructure (DXGI) Device Manager.
        IGNOREHR(pfnMFUnlockDXGIDeviceManager( ));

    }

Cleanup:

    // Release the DXGI Device Manager.
    ReleaseInterface(m_pDXGIDeviceManager);
