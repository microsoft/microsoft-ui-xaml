...
HRESULT CD2DPrintTarget::InitializeImpl()
{
    ...
    wrl::ComPtr<ID3D11DeviceContext> d3d11DeviceContextUnused;
    wrl::ComPtr<ID3D11Device> d3d11Device;
    wrl::ComPtr<ID3D10Multithread> multithread;
    wrl::ComPtr<IDXGIDevice> dxgiDevice;
    wrl::ComPtr<ID2D1Device> d2d1Device;
    wrl::ComPtr<ID2D1Factory1> d2d1Factory1;
    ...
    IFC(D3D11CreateDevice(
                ...
                &d3d11Device,
                &featureLevelUnused,
                &d3d11DeviceContextUnused
                ));
    // Mark the device for multithreading.
    IFC(d3d11Device->QueryInterface(__uuidof(ID3D10Multithread), (void**)&multithread))
    multithread->SetMultithreadProtected(TRUE);

    // Create a D2D device and a D2DDeviceContext
    IFC(d3d11Device.As(&dxgiDevice));
    IFC(m_pFactory->GetFactory()->QueryInterface(__uuidof(ID2D1Factory1), reinterpret_cast<void**>(d2d1Factory1.ReleaseAndGetAddressOf())));
    IFC(d2d1Factory1->CreateDevice(dxgiDevice.Get(), &d2d1Device));
    IFC(d2d1Device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_NONE, &d2d1DeviceContext));

    SetInterface(m_pD2DDeviceContext, d2d1DeviceContext.Get());
    SetInterface(m_pD2D1Device, d2d1Device.Get());
    SetInterface(m_pD3DDevice, d3d11Device.Get());
    ...
    )
{
    ...
    pD2DDeviceContext->SetTarget(nullptr);  // Clears out the ref from the D2D device context to the D3D texture now that we're done drawing.
