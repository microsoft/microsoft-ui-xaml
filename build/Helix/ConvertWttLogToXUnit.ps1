...
Add-Type -Language CSharp -ReferencedAssemblies System.Xml,System.Xml.Linq @"
using System;
                ...
                foreach (XElement element in doc.Root.Elements()) {
                    ...
                    if (element.Name == "StartTest" ) {
                        ...
                        if (testsExecuting == 1) {
                            ...
                            inTestCleanup = false;
                            hasSeenPerfAnalyzerStopSession = false;
                    ...
                    if (currentResult != null && element.Name == "MetadataRef" && hasSeenPerfAnalyzerStopSession) {
                        try {
                            var nameAttrib = element.Attribute("AttributeName");
                            var valueAttrib = element.Attribute("AttributeValue");
                            if (nameAttrib != null && valueAttrib != null) {
                                currentResult.Metrics.Add(new TestMetric() { Name = nameAttrib.Value, Value = Convert.ToDouble(valueAttrib.Value) });
                            }
                        }
                        catch { }
                    }
...
[ConsoleApp1.TestResultParser]::ConvertTaefLogToXUnitLog($TaefInput, $XUnitOutput)