...
### Navigation Flow
- **Tab Navigation**: When navigating to the `SplitMenuFlyoutItem` using keyboard (Tab key), 
    focus moves to the primary button first, then to the flyout button with subsequent Tab presses.
- **Arrow Key Navigation**: Within a menu, the Up/Down arrow keys can navigate between menu items, 
    including the `SplitMenuFlyoutItem`.

### Primary Button Interaction
- **Enter Key**: When the focus is on the primary button, pressing `Enter` executes the 
    primary action (same as clicking the primary button).
- **Space Key**: Similar to `Enter`, pressing `Space` on the primary button executes 
    the primary action.

### Flyout Button Interaction  
...
- **Escape Key**: Closes the submenu and returns focus to the flyout button.
- **Left Arrow Key**: Closes the submenu and returns focus to the flyout button.
