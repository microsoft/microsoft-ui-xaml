...
Today an app (or host) has to turn this on itself. For example, in the Windows shell the
lifted-XAML host ([`LiftedXamlHostBehavior`](https://microsoft.visualstudio.com/OS/_git/os.2020?path=/pcshell/shell/UXFrame/Dll/LiftedXamlHostBehavior.h&version=GBofficial/ge_current_directxbox_dev_staging&line=150&lineEnd=153&lineStartColumn=1&lineEndColumn=14&lineStyle=plain&_a=contents))
enables key routing as part of bringing XAML up (in its `EnsureXamlInitialized` override).
This spec describes doing the equivalent inside the WinUI framework itself, so that a full
WinUI app gets gamepad-driven navigation on by default, while still allowing the app to opt
out.
...
which already has its own reference pages on learn.microsoft.com. That documentation is not
live yet — it ships with the 8D release; see [task.ms/63350910](https://task.ms/63350910) to
track its publication. No new API page is required. The existing surface is listed in
[API Details](#api-details) for reviewer convenience._
